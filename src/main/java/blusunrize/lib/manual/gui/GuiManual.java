...
public class GuiManual extends GuiScreen {
    ...
    public void initGui() {
        ...
        if (manual.getEntry(selectedEntry) != null) {
            ...
            if (mPage != null) mPage.initPage(this, guiLeft + 32, guiTop + 28, pageButtons);
            ...
            for (ManualEntry e : manual.manualContents.values())
                if (manual.showEntryInList(e)) lHeaders.add(e.getName());
            ...
            ArrayList<String> lHeaders = new ArrayList<String>();
            for (String cat : manual.getSortedCategoryList()) if (manual.showCategoryInList(cat)) lHeaders.add(cat);
            headers = lHeaders.toArray(new String[lHeaders.size()]);
        ...
        } else if (searchField != null) searchField = null;
    }
    ...
    public void drawScreen(int mx, int my, float f) {
        ...
        if (manual.getEntry(selectedEntry) != null) {
            ...
            if (page > 0) this.drawTexturedModalRect(guiLeft + 32, guiTop + 179, 0, 216 + (b0 ? 20 : 0), 16, 10);
            ...
            IManualPage mPage = (page < 0 || page >= entry.getPages().length) ? null : entry.getPages()[page];
            if (mPage != null) mPage.renderPage(this, guiLeft + 32, guiTop + 28, mx + guiLeft, my + guiTop);
    ...
    public void actionPerformed(GuiButton button) {
        if (button.id == 0) {
            ...
            if (sel >= 0 && sel < headers.length) {
                if (((GuiClickableList) button).translationType == 0) selectedCategory = headers[sel];
                else {
            ...
            int sel = ((GuiClickableList) button).selectedOption;
            if (sel >= 0 && sel < suggestionHeaders.length) selectedEntry = suggestionHeaders[sel];
            ((GuiClickableList) button).selectedOption = -1;
            this.initGui();
            ...
            ManualEntry entry = manual.getEntry(selectedEntry);
            IManualPage mPage = (page < 0 || page >= entry.getPages().length) ? null : entry.getPages()[page];
            if (mPage != null) mPage.buttonPressed(this, button);
        }
    }
    ...
    protected void mouseMovedOrUp(int mx, int my, int action) {
        ...
        if (backButtonPressed && (action == 0 || action == 1)) backButtonPressed = false;
        lastClick = null;
        lastDrag = null;
    }
