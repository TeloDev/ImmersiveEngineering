...
public class GuiClickableList extends GuiButton {
    ...
    public GuiClickableList(
        ...
        if (perPage < entries.length) maxOffset = entries.length - perPage;
    }
    ...
    public void drawButton(Minecraft mc, int mx, int my) {
        ...
        for (int i = 0; i < Math.min(perPage, entries.length); i++) {
            ...
            if (mmY >= i * getFontHeight() && mmY < (i + 1) * getFontHeight()) col = gui.manual.getHighlightColour();
        ...
        if (mouseWheel != 0 && maxOffset > 0 && Mouse.getEventNanoseconds() != prevWheelNano) {
            ...
            if (mouseWheel < 0 && offset < maxOffset) offset++;
            if (mouseWheel > 0 && offset > 0) offset--;
        }
    }
    ...
    public boolean mousePressed(Minecraft mc, int mx, int my) {
        ...
        if (b) {
            ...
            for (int i = offset; i < Math.min(perPage, entries.length); i++)
                if (mmY >= i * getFontHeight() && mmY < (i + 1) * getFontHeight()) selectedOption = i;
        }
