...
public abstract class ManualPages implements IManualPage {
    ...
    public void initPage(GuiManual gui, int x, int y, List<GuiButton> pageButtons) {
        if (text != null && !text.isEmpty()) {
            ...
            if (this.localizedText == null) this.localizedText = "";
    ...
    public void buttonPressed(GuiManual gui, GuiButton button) {
        if (button instanceof GuiButtonManualLink
                && GuiManual.activeManual != null
                && manual.showEntryInList(manual.getEntry(((GuiButtonManualLink) button).key))) {
            if (GuiManual.previousSelectedEntry.size() > 0)
                GuiManual.previousSelectedEntry.add(0, GuiManual.selectedEntry);
            else GuiManual.previousSelectedEntry.add(GuiManual.selectedEntry);
    ...
    public static class Image extends ManualPages {
        ...
        public void initPage(GuiManual gui, int x, int y, List<GuiButton> pageButtons) {
            ...
            for (int i = 0; i < resources.length; i++)
                if (resources[i] != null && !resources[i].isEmpty()) yOff += sizing[i][3] + 5;
        ...
        public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
            ...
            for (int i = 0; i < resources.length; i++)
                if (resources[i] != null && !resources[i].isEmpty()) {
                    if (resources[i] != lastResource) ManualUtils.bindTexture(resources[i]);
                    int xOff = 60 - sizing[i][2] / 2;
                    ManualUtils.drawTexturedRect(
                            x + xOff,
                            y + yOff,
                            sizing[i][2],
                            sizing[i][3],
                            (sizing[i][0]) / 256f,
                            (sizing[i][0] + sizing[i][2]) / 256f,
                            (sizing[i][1]) / 256f,
                            (sizing[i][1] + sizing[i][3]) / 256f);
                    yOff += sizing[i][3] + 5;
                    lastResource = resources[i];
                }

            if (localizedText != null && !localizedText.isEmpty())
                ManualUtils.drawSplitString(
                        manual.fontRenderer, localizedText, x, y + yOff, 120, manual.getTextColour());
    ...
    public static class Table extends ManualPages {
        ...
        public void initPage(GuiManual gui, int x, int y, List<GuiButton> pageButtons) {
            ...
            manual.fontRenderer.setUnicodeFlag(true);
            int l = localizedText != null
                    ? manual.fontRenderer
                            .listFormattedStringToWidth(localizedText, 120)
                            .size()
                    : 0;
        ...
        public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
            ...
            if (localizedTable != null) {
                ...
                //				gui.drawGradientRect(x,y+textHeight+yOff-2,x+120,y+textHeight+yOff-1,
                ...
                for (int i = 0; i < localizedTable.length; i++) {
                    for (int j = 0; j < localizedTable[i].length; j++)
                        if (localizedTable[i][j] != null) {
                            ...
                            //							manual.fontRenderer.drawSplitString(localizedTable[i][j], xx,y+textHeight+yOff, w,
                            // manual.getTextColour());
                            if (j != 0) {
                                int l = manual.fontRenderer
                                        .listFormattedStringToWidth(localizedTable[i][j], w)
                                        .size();
                                ...
                                if (horizontalBars) {
                                    ...
                                    gui.drawGradientRect(
                                            x,
                                            (int) ((y + textHeight + yOff + l * manual.fontRenderer.FONT_HEIGHT)
                                                    / scale),
                                            x + 120,
                                            (int) ((y + textHeight + yOff + l * manual.fontRenderer.FONT_HEIGHT) / scale
                                                    + 1),
                ...
                if (bars != null)
                    for (int i = 0; i < bars.length; i++)
                        gui.drawGradientRect(
                                textOff[i] - 4, y + textHeight - 4, textOff[i] - 3, y + textHeight + yOff, col, col);
            }
        }
    ...
    public static class ItemDisplay extends ManualPages {
        ...
        public void initPage(GuiManual gui, int x, int y, List<GuiButton> pageButtons) {
            ...
            if (length > 0) {
                ...
                int lines =
                        (length / lineSum * 2) + (length % lineSum / line0) + (length % lineSum % line0 > 0 ? 1 : 0);
        ...
        public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
            ...
            if (length > 0) {
                ...
                for (int line = 0; line < lines; line++) {
                    ...
                    if (line == 0 && perLine > length) perLine = length;
            ...
            if (localizedText != null && !localizedText.isEmpty())
                ManualUtils.drawSplitString(
                        manual.fontRenderer, localizedText, x, y + yOffset, 120, manual.getTextColour());
            //			manual.fontRenderer.drawSplitString(localizedText, x,y+yOffset, 120, manual.getTextColour());

            manual.fontRenderer.setUnicodeFlag(false);
            if (highlighted != null) gui.renderToolTip(highlighted, mx, my);
            RenderHelper.disableStandardItemLighting();
        }
        ...
        public boolean listForSearch(String searchTag) {
            for (ItemStack stack : stacks)
                if (stack.getDisplayName().toLowerCase().contains(searchTag)) return true;
            return false;
        }
    }
    ...
    public static class Crafting extends ManualPages {
            ...
            for (Object o : cmRecipes)
                if (o instanceof IRecipe) {
                    for (int iStack = 0; iStack < stacks.length; iStack++) {
                        ...
                        else checkRecipe((IRecipe) o, stack, stack, iStack);
                    }
                }
        }
        ...
        void checkRecipe(IRecipe rec, Object key, Object stack, int iStack) {
            if (rec.getRecipeOutput() != null && ManualUtils.stackMatchesObject(rec.getRecipeOutput(), stack)) {
                ...
                if (val != null && val.length == 3) {
                    ...
                    int h = (Integer) val[1];
                    if (h * 18 > yOff[iStack]) yOff[iStack] = h * 18;
                }
            }
        }
            ...
            for (Object stack : this.stacks) {
                if (this.recipes.get(stack).size() > 1) {
                    pageButtons.add(new GuiButtonManualNavigation(
                            gui, 100 * i + 0, x - 2, y + yyOff + yOff[i - 1] / 2 - 3, 8, 10, 0));
                    pageButtons.add(new GuiButtonManualNavigation(
                            gui, 100 * i + 1, x + 122 - 16, y + yyOff + yOff[i - 1] / 2 - 3, 8, 10, 1));
                }
                if (this.recipes.get(stack).size() > 0) yyOff += yOff[i - 1] + 8;
                i++;
            }
        ...
        public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
            ...
            for (int i = 0; i < stacks.length; i++) {
                ...
                if (!rList.isEmpty() && recipePage[i] >= 0 && recipePage[i] < this.recipes.size()) {
                    ...
                    for (PositionedItemStack pstack : rList.get(recipePage[i]))
                        if (pstack != null) {
                            if (pstack.x > maxX) maxX = pstack.x;
                            gui.drawGradientRect(
                                    x + pstack.x,
                                    y + totalYOff + pstack.y,
                                    x + pstack.x + 16,
                                    y + totalYOff + pstack.y + 16,
                                    0x33666666,
                                    0x33666666);
                        }
                    ...
                    for (PositionedItemStack pstack : rList.get(recipePage[i]))
                        if (pstack != null)
                            if (pstack.getStack() != null) {
                                ...
                                // ManualUtils.mc().renderEngine, pstack.getStack(), x+pstack.x, y+totalYOff+pstack.y);
                                RenderItem.getInstance()
                                ...
                                if (mx >= x + pstack.x
                                        && mx < x + pstack.x + 16
                                        && my >= y + totalYOff + pstack.y
                                        && my < y + totalYOff + pstack.y + 16) highlighted = pstack.getStack();
                            }
                    totalYOff += yOff[i] + 8;
                }
            }
            ...
            if (localizedText != null && !localizedText.isEmpty())
                ManualUtils.drawSplitString(
                        manual.fontRenderer, localizedText, x, y + totalYOff - 2, 120, manual.getTextColour());
        ...
        public void buttonPressed(GuiManual gui, GuiButton button) {
            ...
            if (r >= 0 && r < stacks.length) {
                if (button.id % 100 == 0) recipePage[r]--;
                else recipePage[r]++;

                if (recipePage[r] >= this.recipes.get(stacks[r]).size()) recipePage[r] = 0;
                if (recipePage[r] < 0)
        ...
        public boolean listForSearch(String searchTag) {
            for (Object stack : stacks) {
                if (stack instanceof ItemStack[]) {
                    for (ItemStack subStack : (ItemStack[]) stack)
                        if (subStack.getDisplayName().toLowerCase().contains(searchTag)) return true;
                } else if (stack instanceof ItemStack) {
                    if (((ItemStack) stack).getDisplayName().toLowerCase().contains(searchTag)) return true;
    ...
    public static class CraftingMulti extends ManualPages {
        ...
        public void recalculateCraftingRecipes() {
            ...
            if (stacks != null && stacks.length > 0 && stacks[0] instanceof PositionedItemStack[]) {
                for (PositionedItemStack[] pisA : (PositionedItemStack[][]) stacks) {
                    for (PositionedItemStack pis : pisA) if (pis != null && pis.y + 18 > yOff) yOff = pis.y + 18;
                ...
                for (Object o : cmRecipes)
                    if (o instanceof IRecipe) {
                        for (int iStack = 0; iStack < stacks.length; iStack++) {
                            Object stack = stacks[iStack];
                            if (((IRecipe) o).getRecipeOutput() != null
                                    && ManualUtils.stackMatchesObject(((IRecipe) o).getRecipeOutput(), stack)) {
                                ...
                                if (val != null && val.length == 3) {
                                    ...
                                    else this.recipes.add((PositionedItemStack[]) val[2]);
                                    int h = (Integer) val[1];
                                    if (h * 18 > yOff) yOff = h * 18;
                                }
                            }
                        }
                    }
        ...
        public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
            ...
            if (!recipes.isEmpty() && recipePage >= 0 && recipePage < this.recipes.size()) {
                ...
                ManualUtils.bindTexture(manual.texture);
                ManualUtils.drawTexturedRect(
                        x + maxX - 17, y + yOff / 2 - 5, 16, 10, 0 / 256f, 16 / 256f, 226 / 256f, 236 / 256f);
            }
                    ...
                    if (pstack != null)
                        if (pstack.getStack() != null) {
                            ...
                            if (mx >= x + pstack.x
                                    && mx < x + pstack.x + 16
                                    && my >= y + pstack.y
                                    && my < y + pstack.y + 16) highlighted = pstack.getStack();
                        }
            }
            ...
            if (localizedText != null && !localizedText.isEmpty())
                ManualUtils.drawSplitString(
                        manual.fontRenderer, localizedText, x, y + yOff + 2, 120, manual.getTextColour());
        ...
        public void buttonPressed(GuiManual gui, GuiButton button) {
            super.buttonPressed(gui, button);
            if (button.id % 100 == 0) recipePage--;
            else recipePage++;

            if (recipePage >= this.recipes.size()) recipePage = 0;
            if (recipePage < 0) recipePage = this.recipes.size() - 1;
        }
            ...
            for (PositionedItemStack[] recipe : this.recipes)
                for (PositionedItemStack stack : recipe) {
                    if (stack.stack instanceof ItemStack[]) {
                        ...
                        if (((ItemStack) stack.stack)
                                .getDisplayName()
                                .toLowerCase()
                                .contains(searchTag)) return true;
    ...
    public static String addLinks(
            ManualInstance helper, GuiManual gui, String text, int x, int y, int width, List<GuiButton> pageButtons) {
        ...
        while ((start = text.indexOf("<link")) >= 0 && overflow < 50) {
            ...
            for (String part : resultParts) repList.add(new String[] {part, segment[1], page});
        ...
        for (int yOff = 0; yOff < list.size(); yOff++) {
            ...
            for (; linkIdx < repList.size(); linkIdx++) {
                ...
                if ((start = s.indexOf(rep[0])) >= lastIdx) {
                    ...
                    pageButtons.add(new GuiButtonManualLink(
                            gui,
                            900 + overflow,
                            x + bx,
                            y + by,
                            bw,
                            (int) (helper.fontRenderer.FONT_HEIGHT * 1.5),
                            bkey,
                            rep[0],
                            bpage));
                    lastIdx = start;
    ...
    public static class PositionedItemStack {
        ...
        public ItemStack getStack() {
            if (displayList == null) {
                ...
                if (stack instanceof ItemStack) {
                    if (((ItemStack) stack).getItem().getHasSubtypes()
                            && ((ItemStack) stack).getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                        ...
                        ((ItemStack) stack)
                                ...
                                .getSubItems(
                                        ...
                                        list);
                        if (list.size() > 0) displayList.addAll(list);
                    } else displayList.add((ItemStack) stack);
                } else if (stack instanceof List && !((List) stack).isEmpty()) {
                    for (ItemStack subStack : (List<ItemStack>) this.stack) {
                        if (subStack.getItem().getHasSubtypes()
                                && subStack.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                            ArrayList<ItemStack> list = new ArrayList<ItemStack>();
                            subStack.getItem()
                                    .getSubItems(
                                            subStack.getItem(),
                                            ...
                                            list);
                            if (list.size() > 0) displayList.addAll(list);
                        } else displayList.add(subStack);
                    }
                }
            }
            if (displayList == null || displayList.isEmpty()) return null;
