            ...
            for (int part = 0; part < sideModels.length - 1; part++)
                if (sideModels[part] != null) {
                    ...
                    for (int pass = 0; pass < maxPasses; pass++)
                        if ((((ShaderCaseMinecart) sCase).additionalTexture != null && pass == 2)
                                ? (part != 1 && part != 2)
                                : (pass != 1 || ((ShaderCaseMinecart) sCase).overlaySides[part])) {
                            int[] col = sCase.getRGBAColourModifier(shader, null, "" + part, pass);
                            boolean upScale = true;
                            GL11.glScalef(scale, scale, scale);
                            GL11.glColor4f(
                                    col[0] / 255f, col[1] / 255f, col[2] / 255f, col.length > 3 ? (col[3] / 255f) : 1f);

                            if (pass == maxPasses - 1)
                                ClientUtils.bindTexture(
                                        "immersiveengineering:textures/models/shaders/minecart_uncoloured.png");
                            else if (pass == maxPasses - 2 && ((ShaderCaseMinecart) sCase).additionalTexture != null)
                                ClientUtils.bindTexture(sCase.getBaseTexturePath()
                                        + ((ShaderCaseMinecart) sCase).additionalTexture + ".png");
                            else if (pass == 0)
                                ClientUtils.bindTexture("immersiveengineering:textures/models/shaders/minecart_0.png");
                            else {
                                ClientUtils.bindTexture(
                                        sCase.getBaseTexturePath() + "1_" + sCase.getOverlayType() + ".png");
                                upScale = false;
                            }
                            ...
                            if (icon != null) {
                                ...
                                if (modelRemapped != null)
                                    ...
                                    else modelRemapped.sideModels[part].render(f5);
                            } else {
                                if (((ShaderCaseMinecart) sCase).mirrorSideForPass[pass])
                                    sideModelsMirrored[part].render(f5);
                                else sideModels[part].render(f5);
                            }
                            sCase.modifyRender(shader, null, "" + part, pass, false, false);

                            GL11.glColor4f(1, 1, 1, 1);
                            GL11.glScalef(1 / scale, 1 / scale, 1 / scale);
                            if (upScale) scale += .001f;
                        }
                }

            GL11.glDisable(GL11.GL_BLEND);
        } else super.render(entity, f0, f1, f2, f3, f4, f5);
    }

    ModelShaderMinecart getRemappedModel(IIcon icon) {
        if (remappedModels.containsKey(icon)) return remappedModels.get(icon);
        ...
        for (ModelRenderer mr : modelRemapped.sideModels)
            if (mr != null) mr.setTextureOffset(mr.textureOffsetX + ox, mr.textureOffsetX + oy);
        ...
        modelRemapped.sideModelsMirrored =
                ClientUtils.copyModelRenderers(modelRemapped, modelRemapped.sideModelsMirrored);
