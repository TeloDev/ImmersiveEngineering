...
public class NEIBlastFurnaceHandler extends TemplateRecipeHandler {
    public class CachedBlastFurnaceRecipe extends CachedRecipe {
        ...
        public CachedBlastFurnaceRecipe(BlastFurnaceRecipe recipe) {
            ...
            if (in instanceof String) in = OreDictionary.getOres((String) in);
            input = new PositionedStack(in, 47, 9);
            output = new PositionedStack(recipe.output, 107, 9);
            if (recipe.slag != null) slag = new PositionedStack(recipe.slag, 107, 45);
            time = recipe.time;

            for (Object fuel : BlastFurnaceRecipe.blastFuels.keySet()) fuels.add(new PositionedStack(fuel, 47, 45));
        }

        @Override
        public List<PositionedStack> getOtherStacks() {
            if (slag != null)
                return Arrays.asList(fuels.get((NEIBlastFurnaceHandler.this.cycleticks / 20) % fuels.size()), slag);
            else return Arrays.asList(fuels.get((NEIBlastFurnaceHandler.this.cycleticks / 20) % fuels.size()));
        }
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId == getOverlayIdentifier()) {
            for (BlastFurnaceRecipe r : BlastFurnaceRecipe.recipeList)
                if (r != null) this.arecipes.add(new CachedBlastFurnaceRecipe(r));
        ...
        if (ingredient != null) {
            ...
            if (r != null) this.arecipes.add(new CachedBlastFurnaceRecipe(r));
                ...
                for (BlastFurnaceRecipe r1 : BlastFurnaceRecipe.recipeList)
                    if (r1 != null) this.arecipes.add(new CachedBlastFurnaceRecipe(r1));
            }
        }
    }
