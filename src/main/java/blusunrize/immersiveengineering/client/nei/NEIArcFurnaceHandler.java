...
public class NEIArcFurnaceHandler extends TemplateRecipeHandler {
    ...
    public static NEIArcFurnaceHandler createSubHandler(String subtype) {
        try {
            ...
            String entityProxySubClassName =
                    NEIArcFurnaceHandler.class.getSimpleName().concat(subtype);

            ClassWriter cw = new ClassWriter(ClassWriter.COMPUTE_MAXS);
            cw.visit(
                    Opcodes.V1_6,
                    Opcodes.ACC_PUBLIC + Opcodes.ACC_SUPER,
                    entityProxySubClassName,
                    null,
            ...
            Class<? extends NEIArcFurnaceHandler> c = (Class<? extends NEIArcFurnaceHandler>)
                    new ProxyClassLoader(Thread.currentThread().getContextClassLoader(), cw.toByteArray())
    ...
    public class CachedArcFurnaceRecipe extends CachedRecipe {
        ...
        public CachedArcFurnaceRecipe(ArcFurnaceRecipe recipe) {
            ...
            if (recipe.input != null) lInputs.add(new PositionedStack(recipe.input, 28, 0));
            ...
            if (recipe.slag != null) slag = new PositionedStack(recipe.slag, 122, 36);
            time = recipe.time;
        ...
        public List<PositionedStack> getOtherStacks() {
            ...
            if (slag != null) l.add(slag);
            if (output != null) for (int i = 1; i < output.length; i++) l.add(output[i]);
            return l;
        }
    ...
    boolean specialTypeCompare(ArcFurnaceRecipe r) {
        if ((this.specialRecipeType == null || this.specialRecipeType.isEmpty())
                && (r.specialRecipeType == null || r.specialRecipeType.isEmpty())) return true;
        if (this.specialRecipeType != null
                && r.specialRecipeType != null
                && this.specialRecipeType.equalsIgnoreCase(r.specialRecipeType)) return true;
        return false;
    }
    ...
    public String getRecipeName() {
        return StatCollector.translateToLocal("tile.ImmersiveEngineering.metalMultiblock.arcFurnace.name")
                + (this.specialRecipeType != null && !this.specialRecipeType.isEmpty()
            ...
            for (ArcFurnaceRecipe r : ArcFurnaceRecipe.recipeList)
                if (r != null
                        ...
                        && (Utils.stackMatchesObject(result, r.output)
                                || (r.slag != null && Utils.stackMatchesObject(result, r.slag)))
                        && specialTypeCompare(r)) this.arecipes.add(new CachedArcFurnaceRecipe(r));
    }
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        if (ingredient != null)
            for (ArcFurnaceRecipe r : ArcFurnaceRecipe.recipeList)
                if (r != null && r.input != null && specialTypeCompare(r)) {
                    if (Utils.stackMatchesObject(ingredient, r.input)) this.arecipes.add(new CachedArcFurnaceRecipe(r));
                    else
    ...
    public void drawBackground(int recipe) {
        ...
        if (r != null) {
            ...
            for (int i = 0; i < 4; i++) ClientUtils.drawSlot(20 + i % 2 * 18, 24 + i / 2 * 18, 16, 16);
            if (r.output != null) for (PositionedStack ps : r.output) ClientUtils.drawSlot(ps.relx, ps.rely, 16, 16);
            if (r.slag != null) ClientUtils.drawSlot(r.slag.relx, r.slag.rely, 16, 16);
