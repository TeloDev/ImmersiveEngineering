...
public class GuiAssembler extends GuiContainer {
    ...
    public void drawScreen(int mx, int my, float partial) {
        ...
        ClientUtils.handleGuiTank(
                ...
                mx,
                my,
                "immersiveengineering:textures/gui/assembler.png",
                tooltip);
        ClientUtils.handleGuiTank(
                tile.tanks[1],
                guiLeft + 182,
            ...
            if (tile.inventory[18 + i] == null && tile.patterns[i].inv[9] != null)
                if (mx >= guiLeft + 27 + i * 58
                    ...
                    tile.patterns[i]
                            .inv[9]
                            .getItem()
                            .addInformation(tile.patterns[i].inv[9], ClientUtils.mc().thePlayer, tooltip, false);
                    for (int j = 0; j < tooltip.size(); j++)
                        tooltip.set(
                                j,
                                (j == 0 ? tile.patterns[i].inv[9].getRarity().rarityColor : EnumChatFormatting.GRAY)
                                        + tooltip.get(j));
                }
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int mx, int my) {
        ...
        int stored =
                (int) (46 * (tile.energyStorage.getEnergyStored() / (float) tile.energyStorage.getMaxEnergyStored()));
        ClientUtils.drawGradientRect(
                guiLeft + 187, guiTop + 13 + (46 - stored), guiLeft + 194, guiTop + 59, 0xffb51500, 0xff600b00);
        ...
        for (int i = 0; i < tile.patterns.length; i++)
            if (tile.inventory[18 + i] == null && tile.patterns[i].inv[9] != null) {
                ...
                itemRender.renderItemAndEffectIntoGUI(
                        font, this.mc.getTextureManager(), stack, guiLeft + 27 + i * 58, guiTop + 64);
                itemRender.renderItemOverlayIntoGUI(
                        ...
                        guiLeft + 27 + i * 58,
                        guiTop + 64,
                        EnumChatFormatting.GRAY.toString() + stack.stackSize);
                this.zLevel = 0.0F;
