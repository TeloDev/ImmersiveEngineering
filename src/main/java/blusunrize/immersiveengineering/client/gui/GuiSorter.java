...
public class GuiSorter extends GuiContainer {
    ...
    public void drawScreen(int mx, int my, float partial) {
        ...
        for (GuiButton button : (List<GuiButton>) this.buttonList) {
            if (button instanceof ButtonSorter)
                if (mx > button.xPosition
                        && mx < button.xPosition + 18
                        && my > button.yPosition
                        && my < button.yPosition + 18) {
                    ArrayList<String> tooltip = new ArrayList<String>();
                    int type = ((ButtonSorter) button).type;
                    String[] split = StatCollector.translateToLocal(
                                    Lib.DESC_INFO + "filter." + (type == 0 ? "oreDict" : type == 1 ? "nbt" : "fuzzy"))
                            .split("<br>");
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int mx, int my) {
        ...
        for (int side = 0; side < 6; side++) {
            ...
            String s = StatCollector.translateToLocal("desc.ImmersiveEngineering.info.blockSide."
                            + ForgeDirection.getOrientation(side).toString())
                    .substring(0, 1);
            GL11.glEnable(3042);
    ...
    public void initGui() {
        ...
        for (int side = 0; side < 6; side++)
            for (int i = 0; i < 3; i++) {
                ...
                b.active =
                        i == 0 ? this.tile.doOredict(side) : i == 1 ? this.tile.doNBT(side) : this.tile.doFuzzy(side);
    ...
    public static class ButtonSorter extends GuiButton {
        ...
        public void drawButton(Minecraft mc, int mx, int my) {
            if (this.visible) {
                GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
                this.field_146123_n = mx >= this.xPosition
                        && my >= this.yPosition
                ...
                this.drawTexturedModalRect(
                        this.xPosition, this.yPosition, 176 + type * 18, (active ? 3 : 21), this.width, this.height);
