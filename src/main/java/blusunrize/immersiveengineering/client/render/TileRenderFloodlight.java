...
public class TileRenderFloodlight extends TileRenderImmersiveConnectable {
    ModelIEObj model = new ModelIEObj("immersiveengineering:models/floodlight.obj") {
        ...
        public IIcon getBlockIcon(String groupName) {
            return IEContent.blockMetalDevice2.getIcon(
                    groupName.equals("glass") ? 1 : 0, BlockMetalDevices2.META_floodlight);
        }
    };
    ...
    public void renderStatic(TileEntity tile, Tessellator tes, Matrix4 translationMatrix, Matrix4 rotationMatrix) {
        ...
        translationMatrix.translate(
                .5 + (light.side == 4 ? -.125 : light.side == 5 ? .125 : 0),
                .5 + (light.side == 0 ? -.125 : light.side == 1 ? .125 : 0),
                .5 + (light.side == 2 ? -.125 : light.side == 3 ? .125 : 0));
        if (light.side == 0) rotationMatrix.rotate(Math.PI, light.facing < 4 ? 0 : 1, 0, light.facing < 4 ? 1 : 0);
        else if (light.side != 1)
            rotationMatrix.rotate(
                    Math.PI / 2,
                    light.side == 2 ? -1 : light.side == 3 ? 1 : 0,
                    0,
                    light.side == 5 ? -1 : light.side == 4 ? 1 : 0);
        ...
        String[] s = {"light", "off"};
        if (BlockRenderMetalDevices2.renderPass == 1) s = new String[] {"glass"};
        else if (((TileEntityFloodlight) tile).active) s[1] = "on";
