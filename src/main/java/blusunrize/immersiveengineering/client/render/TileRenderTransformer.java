    ...
    public void renderStatic(TileEntity tile, Tessellator tes, Matrix4 translationMatrix, Matrix4 rotationMatrix) {
        ...
        if (transformer.postAttached != 0) {
            ...
            if (transformer instanceof TileEntityTransformerHV) {
                if (transformer.getLimiter(0) == WireType.STEEL) list.add("ConnectorHV_Left");
                else list.add("Connector_Left");
                if (transformer.getLimiter(1) == WireType.STEEL) list.add("ConnectorHV_Right");
                else list.add("Connector_Right");
            } else {
