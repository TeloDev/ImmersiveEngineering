...
public class TileRenderDieselGenerator extends TileRenderIE {
    ...
    public void renderStatic(TileEntity tile, Tessellator tes, Matrix4 translationMatrix, Matrix4 rotationMatrix) {
        ...
        rotationMatrix.rotate(
                Math.toRadians(gen.facing == 3 ? 180 : gen.facing == 4 ? 90 : gen.facing == 5 ? -90 : 0), 0, 1, 0);
        if (gen.mirrored) translationMatrix.scale(new Vertex(gen.facing < 4 ? -1 : 1, 1, gen.facing > 3 ? -1 : 1));
    ...
    public void renderDynamic(TileEntity tile, double x, double y, double z, float f) {
        TileEntityDieselGenerator gen = (TileEntityDieselGenerator) tile;
        if (!gen.formed || gen.pos != 31) return;
        GL11.glPushMatrix();
