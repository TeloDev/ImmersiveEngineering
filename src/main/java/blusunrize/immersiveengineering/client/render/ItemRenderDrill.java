...
public class ItemRenderDrill implements IItemRenderer {
    ...
    public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
        ...
        if (type != ItemRenderType.EQUIPPED_FIRST_PERSON) GL11.glRotatef(-45, 0, 1, 0);
        ...
        if (upgrades.getBoolean("waterproof")) parts[2] = "upgrade_waterproof";
        if (upgrades.getInteger("speed") > 0) parts[3] = "upgrade_speed";
        ...
        ShaderCase sCase = (shader != null && shader.getItem() instanceof IShaderItem)
                ? ((IShaderItem) shader.getItem()).getShaderCase(shader, item, "drill")
                : null;

        if (sCase == null) ClientUtils.renderWavefrontWithIconUVs(model, icon, parts);
        else {
        ...
        if (head != null) {
            if (type == ItemRenderType.INVENTORY
                    && !((ItemDrill) item.getItem()).canDrillBeUsed(item, ClientUtils.mc().thePlayer)) angle = 0;
