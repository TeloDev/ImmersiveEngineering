...
public class ExcavatorHandler {
    ...
    public static void recalculateChances(boolean mutePackets) {
        ...
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) {
            ...
            if (!mutePackets) ImmersiveEngineering.packetHandler.sendToAll(new MessageMineralListSync(packetMap));
        }
    }
    ...
    public static MineralMix getRandomMineral(World world, int chunkX, int chunkZ) {
        ...
        if (info == null || (info.mineral == null && info.mineralOverride == null)) return null;

        if (mineralVeinCapacity >= 0 && info.depletion > mineralVeinCapacity) return null;
    ...
    public static class MineralMix {
        ...
        public ItemStack getRandomOre(Random rand) {
            ...
            for (int i = 0; i < recalculatedChances.length; i++) {
                ...
                if (r < 0) return this.oreOutput[i];
            }
            return null;
        }
        ...
        public boolean validDimension(int dim) {
            if (dimensionWhitelist != null && dimensionWhitelist.length > 0) {
                for (int white : dimensionWhitelist) if (dim == white) return true;
                return false;
            } else if (dimensionBlacklist != null && dimensionBlacklist.length > 0) {
                for (int black : dimensionBlacklist) if (dim == black) return false;
                return true;
            }
            return true;
        }
        ...
        public NBTTagCompound writeToNBT() {
            ...
            for (String ore : this.ores) tagList.appendTag(new NBTTagString(ore));
            tag.setTag("ores", tagList);

            tagList = new NBTTagList();
            for (float chance : this.chances) tagList.appendTag(new NBTTagFloat(chance));
            tag.setTag("chances", tagList);

            tagList = new NBTTagList();
            if (this.oreOutput != null)
                for (ItemStack output : this.oreOutput) tagList.appendTag(output.writeToNBT(new NBTTagCompound()));
            tag.setTag("oreOutput", tagList);

            tagList = new NBTTagList();
            for (float chance : this.recalculatedChances) tagList.appendTag(new NBTTagFloat(chance));
        ...
        public static MineralMix readFromNBT(NBTTagCompound tag) {
            ...
            for (int i = 0; i < ores.length; i++) ores[i] = tagList.getStringTagAt(i);

            tagList = tag.getTagList("chances", 5);
            float[] chances = new float[tagList.tagCount()];
            for (int i = 0; i < chances.length; i++) chances[i] = tagList.func_150308_e(i);
            ...
            for (int i = 0; i < recalculatedChances.length; i++) recalculatedChances[i] = tagList.func_150308_e(i);
    ...
    public static class MineralWorldInfo {
        ...
        public NBTTagCompound writeToNBT() {
            NBTTagCompound tag = new NBTTagCompound();
            if (mineral != null) tag.setString("mineral", mineral.name);
            if (mineralOverride != null) tag.setString("mineralOverride", mineralOverride.name);
        ...
        public static MineralWorldInfo readFromNBT(NBTTagCompound tag) {
            ...
            if (tag.hasKey("mineral")) {
                ...
                for (MineralMix mineral : mineralList.keySet())
                    if (s.equalsIgnoreCase(mineral.name)) info.mineral = mineral;
            }
            if (tag.hasKey("mineralOverride")) {
                String s = tag.getString("mineralOverride");
                for (MineralMix mineral : mineralList.keySet())
                    if (s.equalsIgnoreCase(mineral.name)) info.mineralOverride = mineral;
            }
        ...
        if (ManualHelper.getManual() != null) {
            ...
            Collections.sort(mineralIndices, new Comparator<Integer>() {
                ...
                public int compare(Integer paramT1, Integer paramT2) {
                    ...
                    if (localizedName1 == name1) localizedName1 = minerals[paramT1].name;

                    String name2 = Lib.DESC_INFO + "mineral." + minerals[paramT2].name;
                    String localizedName2 = StatCollector.translateToLocal(name2);
                    if (localizedName2 == name2) localizedName2 = minerals[paramT2].name;
            ...
            for (int i : mineralIndices) {
                ...
                if (localizedName == name) localizedName = minerals[i].name;
