...
public class ShaderCaseRevolver extends ShaderCase {
    ...
    public int getPasses(ItemStack shader, ItemStack item, String modelPart) {
        ...
        if (modelPart.equals("cosmetic_compensator")) return 1 + i;
        if (modelPart.equals("bayonet_attachment")
                || modelPart.equals("player_bayonet")
                || modelPart.equals("dev_bayonet")
                || modelPart.equals("player_mag")
                || modelPart.equals("dev_mag")) return 2 + i;
        return 3 + i;
    }
    ...
    public IIcon getReplacementIcon(ItemStack shader, ItemStack item, String modelPart, int pass) {
        ...
        boolean hasUncoloured = modelPart.equals("barrel")
                || modelPart.equals("dev_scope")
                || modelPart.equals("player_mag")
                || modelPart.equals("dev_mag")
                || modelPart.equals("player_electro_0")
                || modelPart.equals("player_electro_1");
        if (pass == maxPass - 1 && hasUncoloured) // uncoloured
        return i_revolverUncoloured;
        if (pass == maxPass - (hasUncoloured ? 2 : 1) && i_revolverAdditional != null) return i_revolverAdditional;
    ...
    public int[] getRGBAColourModifier(ItemStack shader, ItemStack item, String modelPart, int pass) {
        ...
        if (pass == maxPass - (hasUncoloured ? 2 : 1) && i_revolverAdditional != null) return colourOverlay;

        int i = getTextureType(modelPart, pass); // 0 == grip, 1==main, 2==detail, 3==blade
        if (i == 0) return colourUnderlying;
        if (i == 1) return colourPrimary;
        if (i == 2) return colourSecondary;
        if (i == 3) return colourBlade;
    ...
    public void modifyRender(
        ...
        if (modelPart.equals("cosmetic_compensator")) {
            if (pre) GL11.glDisable(GL11.GL_CULL_FACE);
