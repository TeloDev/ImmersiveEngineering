...
public class ShaderRegistry {
    ...
    public static HashMap<String, HashMap<EnumRarity, Integer>> playerTotalWeight =
            new HashMap<String, HashMap<EnumRarity, Integer>>();

    public static ShaderCase getShader(String name, String shaderType) {
        if (shaderRegistry.containsKey(name)) return shaderRegistry.get(name).getCase(shaderType);
        return null;
    }

    public static ShaderRegistryEntry registerShader(
            String name,
            String overlayType,
            EnumRarity rarity,
            int[] colourPrimary,
        ...
        registerShader_Chemthrower(
                ...
                false,
                additionalTexture);
        registerShader_Drill(
                name, overlayType, rarity, colourBackground, colourPrimary, colourSecondary, additionalTexture);
        registerShader_Railgun(
                name, overlayType, rarity, colourBackground, colourPrimary, colourSecondary, additionalTexture);
        registerShader_Minecart(
                name, overlayType, rarity, colourBackground, colourPrimary, colourSecondary, additionalTexture);
        registerShader_Balloon(
                name, overlayType, rarity, colourBackground, colourPrimary, colourSecondary, additionalTexture);
        return shaderRegistry.get(name).setCrateLoot(loot).setBagLoot(bags);
    }

    public static ShaderCaseRevolver registerShader_Revolver(
            String name,
            String overlayType,
            EnumRarity rarity,
            int[] colourGrip,
            ...
            int[] colourBlade,
            String additionalTexture) {
        if (!shaderList.contains(name)) shaderList.add(name);
        ShaderCaseRevolver shader = new ShaderCaseRevolver(
                overlayType, colourGrip, colourPrimary, colourSecondary, colourBlade, additionalTexture);
        if (!shaderRegistry.containsKey(name)) shaderRegistry.put(name, new ShaderRegistryEntry(name, rarity, shader));
        else shaderRegistry.get(name).addCase(shader);
        return shader;
    }

    public static ShaderCaseChemthrower registerShader_Chemthrower(
            String name,
            String overlayType,
            EnumRarity rarity,
            int[] colourGrip,
            ...
            String additionalTexture) {
        if (!shaderList.contains(name)) shaderList.add(name);
        ShaderCaseChemthrower shader = new ShaderCaseChemthrower(
                overlayType,
                colourGrip,
                colourPrimary,
                colourSecondary,
    ...
    public static ShaderCaseDrill registerShader_Drill(
            String name,
            String overlayType,
            EnumRarity rarity,
            int[] colourGrip,
            ...
            String additionalTexture) {
        if (!shaderList.contains(name)) shaderList.add(name);
        ShaderCaseDrill shader =
                new ShaderCaseDrill(overlayType, colourGrip, colourPrimary, colourSecondary, additionalTexture);
        if (!shaderRegistry.containsKey(name)) shaderRegistry.put(name, new ShaderRegistryEntry(name, rarity, shader));
        else shaderRegistry.get(name).addCase(shader);
    ...
    public static ShaderCaseRailgun registerShader_Railgun(
            String name,
            String overlayType,
            EnumRarity rarity,
            int[] colourGrip,
            ...
            String additionalTexture) {
        if (!shaderList.contains(name)) shaderList.add(name);
        ShaderCaseRailgun shader =
                new ShaderCaseRailgun(overlayType, colourGrip, colourPrimary, colourSecondary, additionalTexture);
        if (!shaderRegistry.containsKey(name)) shaderRegistry.put(name, new ShaderRegistryEntry(name, rarity, shader));
        else shaderRegistry.get(name).addCase(shader);
    ...
    public static ShaderCaseMinecart registerShader_Minecart(
            String name,
            String overlayType,
            EnumRarity rarity,
            ...
            String additionalTexture) {
        if (!shaderList.contains(name)) shaderList.add(name);
        ShaderCaseMinecart shader = new ShaderCaseMinecart(
                overlayType, colourUnderlying, colourPrimary, colourSecondary, additionalTexture);
        if (!shaderRegistry.containsKey(name)) shaderRegistry.put(name, new ShaderRegistryEntry(name, rarity, shader));
        else shaderRegistry.get(name).addCase(shader);
    ...
    public static ShaderCaseBalloon registerShader_Balloon(
            String name,
            String overlayType,
            EnumRarity rarity,
            int[] colourUnderlying,
            ...
            String additionalTexture) {
        if (!shaderList.contains(name)) shaderList.add(name);
        ShaderCaseBalloon shader =
                new ShaderCaseBalloon(overlayType, colourUnderlying, colourPrimary, colourSecondary, additionalTexture);
        if (!shaderRegistry.containsKey(name)) shaderRegistry.put(name, new ShaderRegistryEntry(name, rarity, shader));
        else shaderRegistry.get(name).addCase(shader);
        ...
        Collections.sort(sortedRarityMap, new Comparator<EnumRarity>() {
            ...
            public int compare(EnumRarity enum0, EnumRarity enum1) {
                return Integer.compare(
                        ShaderRegistry.rarityWeightMap.get(enum0), ShaderRegistry.rarityWeightMap.get(enum1));
            }
        });
                ...
                if (!upperRarities.isEmpty()) {
                    ...
                    if (!inputList.isEmpty())
                        recipes.add(new PositionedItemStack[] {
                            new PositionedItemStack(inputList, 33, 0), new PositionedItemStack(s0, 69, 0)
                        });
                    ...
                    if (!inputList.isEmpty())
                        recipes.add(new PositionedItemStack[] {
                            new PositionedItemStack(inputList, 33, 0), new PositionedItemStack(s1, 69, 0)
                        });
                }
            }
            manualEntry.getPages()[1] = new ManualPages.ItemDisplay(ManualHelper.getManual(), "shader1", shaderBags);
            manualEntry.getPages()[2] = new ManualPages.CraftingMulti(ManualHelper.getManual(), "shader2", (Object[])
                    recipes.toArray(new PositionedItemStack[recipes.size()][3]));
        }
    }

    public static void recalculatePlayerTotalWeight(String player) {
        if (!playerTotalWeight.containsKey(player)) playerTotalWeight.put(player, new HashMap<EnumRarity, Integer>());
        else playerTotalWeight.get(player).clear();
            ...
            if (entry.getIsBagLoot()) {
                ...
                for (Map.Entry<EnumRarity, Integer> weightedRarity : rarityWeightMap.entrySet())
                    if (entry.getIsInLowerBags()
                            ? (weightedRarity.getValue() >= entryRarityWeight)
                            : (weightedRarity.getValue() == entryRarityWeight)) {
                        int weight = playerTotalWeight.get(player).containsKey(weightedRarity.getKey())
                                ? playerTotalWeight.get(player).get(weightedRarity.getKey())
                                : 0;
                        int value = entry.getWeight();
                        if (received.contains(entry.getName())) value = 1;
    ...
    public static String getRandomShader(String player, Random rand, EnumRarity minRarity, boolean addToReceived) {
        int total = 0;
        if (!playerTotalWeight.containsKey(player)) playerTotalWeight.put(player, totalWeight);
        ...
        for (ShaderRegistryEntry entry : shaderRegistry.values())
            if (entry.getIsBagLoot()) {
                ...
                if (entry.getIsInLowerBags() ? (minWeight >= entryRarityWeight) : (minWeight == entryRarityWeight)) {
                    int value = entry.getWeight();
                    if (receivedShaders.get(player).contains(entry.getName())) value = 1;
                    weight -= value;
                    if (weight <= 0) {
        ...
        if (addToReceived && shader != null && !shader.isEmpty() && player != null && !player.isEmpty()) {
            if (!receivedShaders.get(player).contains(shader)) receivedShaders.put(player, shader);
    ...
    public static EnumRarity getLowerRarity(EnumRarity rarity) {
        ...
        for (int next = idx + 1; next < sortedRarityMap.size(); next++)
            if (rarityWeightMap.get(sortedRarityMap.get(next)) > weight) return sortedRarityMap.get(next);
        return null;
    }
    ...
    public static ArrayList<EnumRarity> getAllLowerRarities(EnumRarity rarity) {
        ...
        int weight = rarityWeightMap.get(rarity);
        for (int next = idx + 1; next < sortedRarityMap.size(); next++)
            if (rarityWeightMap.get(sortedRarityMap.get(next)) > weight) list.add(sortedRarityMap.get(next));
        return list;
    }
    ...
    public static class ShaderRegistryEntry {
        ...
        public ShaderRegistryEntry(String name, EnumRarity rarity, List<ShaderCase> cases) {
            ...
            for (ShaderCase sCase : cases) this.cases.put(sCase.getShaderType(), sCase);
        }
