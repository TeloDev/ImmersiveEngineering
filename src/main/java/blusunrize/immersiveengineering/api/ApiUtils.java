...
public class ApiUtils {
    ...
    public static boolean stackMatchesObject(ItemStack stack, Object o) {
        if (o instanceof ItemStack) return OreDictionary.itemMatches((ItemStack) o, stack, false);
            ...
            for (Object io : (ArrayList) o)
                if (io instanceof ItemStack && OreDictionary.itemMatches((ItemStack) io, stack, false)) return true;
    ...
    public static ComparableItemStack createComparableItemStack(ItemStack stack) {
        ...
        if (stack.hasTagCompound()) comp.setUseNBT(true);
        return comp;
    }
    ...
    public static Object convertToValidRecipeInput(Object input) {
        if (input instanceof ItemStack) return input;
        else if (input instanceof Item) return new ItemStack((Item) input);
        else if (input instanceof Block) return new ItemStack((Block) input);
        else if (input instanceof ArrayList) return input;
        else if (input instanceof String) {
            if (!ApiUtils.isExistingOreName((String) input)) return null;
    ...
    public static ItemStack getItemStackFromObject(Object o) {
        ...
        else if (o instanceof ArrayList) return ((ArrayList<ItemStack>) o).get(0);
        else if (o instanceof String) {
            if (!isExistingOreName((String) o)) return null;
    ...
    static class ValueComparator implements Comparator<String> {
        ...
        public int compare(String s0, String s1) {
            if (inverse) {
                if (base.get(s0) <= base.get(s1)) return -1;
                else return 1;
            } else {
                if (base.get(s0) >= base.get(s1)) return -1;
                else return 1;
