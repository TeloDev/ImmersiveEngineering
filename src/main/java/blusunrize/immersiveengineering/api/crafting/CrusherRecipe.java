...
public class CrusherRecipe {
    ...
    public CrusherRecipe addToSecondaryOutput(Object... outputs) {
        ...
        int i = 0;
        for (Float f : newSecondaryChance) secondaryChance[i++] = f;
        return this;
    }
    ...
    public static CrusherRecipe addRecipe(ItemStack output, Object input, int energy) {
        ...
        if (r.input != null && r.output != null) recipeList.add(r);
        return r;
    }

    public static CrusherRecipe findRecipe(ItemStack input) {
        for (CrusherRecipe recipe : recipeList) if (ApiUtils.stackMatchesObject(input, recipe.input)) return recipe;
        return null;
    }
