...
public class ComparableItemStack {
    ...
    public ComparableItemStack(ItemStack stack) {
        ...
        if (oids != null && oids.length > 0) this.oreID = oids[0];
    }
    ...
    public boolean equals(Object object) {
        if (!(object instanceof ComparableItemStack)) return false;
        ...
        if (!OreDictionary.itemMatches(stack, otherStack, false)) return false;
        if (this.useNBT) if (!ItemStack.areItemStackTagsEqual(stack, otherStack)) return false;
        return true;
    }
}
