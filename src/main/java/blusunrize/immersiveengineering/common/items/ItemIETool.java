...
public class ItemIETool extends ItemIEBase implements cofh.api.item.IToolHammer, ITool {
        ...
        if (ItemNBTHelper.hasKey(stack, "linkingPos")) {
            ...
            if (link != null && link.length > 3)
                list.add(StatCollector.translateToLocalFormatted(
                        Lib.DESC_INFO + "attachedToDim", link[1], link[2], link[3], link[0]));
        }
    }
    ...
    public boolean onItemUseFirst(
        ...
        if (!world.isRemote) {
            ...
            // override:
            // "+(info.mineralOverride!=null?EnumChatFormatting.GREEN+info.mineralOverride.name+EnumChatFormatting.RESET:"none")+"  depletion: "+info.depletion));
            if (stack.getItemDamage() == 0) {
                ...
                ImmersiveNetHandler.INSTANCE.clearAllConnectionsFor(
                        Utils.toCC(nodeHere), world, new TargetingInfo(side, hitX, hitY, hitZ));
                ...
                if (!player.isSneaking()
                    ...
                    if (max > 0)
                        player.addChatMessage(
                                new ChatComponentTranslation(Lib.CHAT_INFO + "energyStorage", stored, max));
                    return true;
                }
                if (player.isSneaking() && tileEntity instanceof IImmersiveConnectable) {
                    if (!ItemNBTHelper.hasKey(stack, "linkingPos"))
                        ...
                        if (pos[0] == world.provider.dimensionId) {
                            ...
                            Set<AbstractConnection> connections =
                                    ImmersiveNetHandler.INSTANCE.getIndirectEnergyConnections(
                                            Utils.toCC(nodeLink), world);
    ...
    public boolean doesSneakBypassUse(World world, int x, int y, int z, EntityPlayer player) {
        if (player.getCurrentEquippedItem() != null
                && this.equals(player.getCurrentEquippedItem().getItem()))
    ...
    public Set<String> getToolClasses(ItemStack stack) {
        ...
        return meta == 0
                ? ImmutableSet.of(Lib.TOOL_HAMMER)
