...
public class ItemRevolver extends ItemUpgradeableTool implements IShaderEquipableItem, ITool {
    ...
    public String getUnlocalizedName(ItemStack stack) {
        if (stack.getItemDamage() != 1) {
            String tag = getRevolverDisplayTag(stack);
            if (!tag.isEmpty()) return this.getUnlocalizedName() + "." + tag;
        }
    ...
    public void onUpdate(ItemStack stack, World world, Entity ent, int slot, boolean inHand) {
        if (!world.isRemote && stack.getItemDamage() != 1 && ent != null && ItemNBTHelper.hasKey(stack, "blocked")) {
            ...
            if (l == 0) ItemNBTHelper.setDelayedSoundsForStack(stack, "cylinderClose", "fire.ignite", .6f, 5, 1, 6, 1);
            l = ItemNBTHelper.handleDelayedSoundsForStack(stack, "cylinderClose", ent);
            if (l == 0) ItemNBTHelper.setDelayedSoundsForStack(stack, "cylinderSpin", "note.hat", .1f, 5, 5, 8, 1);
            l = ItemNBTHelper.handleDelayedSoundsForStack(stack, "cylinderSpin", ent);
            if (l == 0) ItemNBTHelper.remove(stack, "blocked");
        }
    }
        ...
        if (!world.isRemote) {
            if (player.isSneaking() || revolver.getItemDamage() == 1)
                ...
                if (!ItemNBTHelper.getBoolean(revolver, "blocked")) {
                    ...
                    } else world.playSoundAtEntity(player, "note.hat", .6f, 3);
    ...
    public boolean isEmpty(ItemStack stack) {
        ...
        boolean empty = true;
        for (ItemStack b : bullets)
            if (b != null && b.getItem() instanceof IBullet && ((IBullet) b.getItem()).canSpawnBullet(b)) empty = false;
        return empty;
    }
    ...
    public void setBullets(ItemStack revolver, ItemStack[] bullets) {
        ItemStack[] stackList = this.getContainedItems(revolver);
        for (int i = 0; i < bullets.length; i++) stackList[i] = bullets[i];
    ...
    public IIcon getRevolverIcon(ItemStack revolver) {
        String tag = ItemNBTHelper.getString(revolver, "elite");
        if (!tag.isEmpty()) return this.revolverIcons.get(tag);
        else return this.revolverDefaultTexture;
    }
    ...
    public String[] compileRender(ItemStack revolver) {
        ...
        if (tag != null && !tag.isEmpty() && specialRevolversByTag.containsKey(tag)) {
            ...
            if (r != null && r.renderAdditions != null) for (String ss : r.renderAdditions) render.add(ss);
        ...
        if (upgrades.getInteger("bullets") > 0 && !render.contains("dev_mag")) render.add("player_mag");
    ...
    public void onCreated(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (specialRevolvers.containsKey(uuid)) {
            ...
            if (!list.isEmpty()) {
                ...
                if (existingTag.isEmpty()) applySpecialCrafting(stack, list.get(0));
                else {
                    int i = 0;
                    for (; i < list.size(); i++) if (list.get(i) != null && existingTag.equals(list.get(i).tag)) break;
    ...
    public void applySpecialCrafting(ItemStack stack, SpecialRevolver r) {
        ...
        if (r.tag != null && !r.tag.isEmpty()) ItemNBTHelper.setString(stack, "elite", r.tag);
        if (r.flavour != null && !r.flavour.isEmpty()) ItemNBTHelper.setString(stack, "flavour", r.flavour);
        NBTTagCompound baseUpgrades = new NBTTagCompound();
        for (Map.Entry<String, Object> e : r.baseUpgrades.entrySet()) {
            if (e.getValue() instanceof Boolean) baseUpgrades.setBoolean(e.getKey(), (Boolean) e.getValue());
            else if (e.getValue() instanceof Integer) baseUpgrades.setInteger(e.getKey(), (Integer) e.getValue());
            else if (e.getValue() instanceof Float) baseUpgrades.setDouble(e.getKey(), (Float) e.getValue());
            else if (e.getValue() instanceof Double) baseUpgrades.setDouble(e.getKey(), (Double) e.getValue());
            else if (e.getValue() instanceof String) baseUpgrades.setString(e.getKey(), (String) e.getValue());
        }
    ...
    public void removeFromWorkbench(EntityPlayer player, ItemStack stack) {
        ...
        if (contents[18] != null && contents[19] != null) player.triggerAchievement(IEAchievements.upgradeRevolver);
    }
