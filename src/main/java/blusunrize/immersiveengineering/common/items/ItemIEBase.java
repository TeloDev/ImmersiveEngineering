...
public class ItemIEBase extends Item {
    ...
    public ItemIEBase setMetaHidden(int... array) {
        for (int i : array) if (i >= 0 && i < hiddenMeta.length) this.hiddenMeta[i] = true;
        return this;
    }

    public ItemIEBase setMetaUnhidden(int... array) {
        for (int i : array) if (i >= 0 && i < hiddenMeta.length) this.hiddenMeta[i] = false;
        return this;
    }
    ...
    public void registerIcons(IIconRegister ir) {
        ...
        else this.icons[0] = ir.registerIcon("immersiveengineering:" + itemName);
    }

    @Override
    @SideOnly(Side.CLIENT)
    public IIcon getIconFromDamage(int meta) {
        if (getSubNames() != null) if (meta >= 0 && meta < icons.length) return this.icons[meta];
        return icons[0];
    }
    ...
    public void getSubItems(Item item, CreativeTabs tab, List list) {
        if (getSubNames() != null) {
            for (int i = 0; i < getSubNames().length; i++)
                if ((i >= 0 && i < hiddenMeta.length) ? !hiddenMeta[i] : true) list.add(new ItemStack(this, 1, i));
