...
public class ItemChemthrower extends ItemUpgradeableTool implements IShaderEquipableItem, IFluidContainerItem, ITool {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (player.isSneaking()) {
            if (!world.isRemote) ItemNBTHelper.setBoolean(stack, "ignite", !ItemNBTHelper.getBoolean(stack, "ignite"));
        ...
        if (fs != null && fs.getFluid() != null) {
            ...
            if (consumed * duration <= fs.amount) {
                ...
                for (int i = 0; i < split; i++) {
                    ...
                    if (ignite) chem.setFire(10);
                    if (!player.worldObj.isRemote) player.worldObj.spawnEntityInWorld(chem);
                }
                if (count % 4 == 0) {
                    if (ignite) player.playSound("immersiveengineering:spray_fire", .5f, 1.5f);
                    else player.playSound("immersiveengineering:spray", .5f, .75f);
                }
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World world, EntityPlayer player, int timeLeft) {
        ...
        if (fs != null) {
            ...
            if (fs.amount <= 0) ItemNBTHelper.remove(stack, "fluid");
            else ItemNBTHelper.setFluidStack(stack, "fluid", fs);
        }
    }
