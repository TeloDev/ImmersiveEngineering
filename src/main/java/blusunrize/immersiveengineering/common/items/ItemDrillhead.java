...
public class ItemDrillhead extends ItemIEBase implements IDrillHead {
    ...
    private void addPerm(int i, DrillHeadPerm perm) {
        if (i < perms.length) perms[i] = perm;
    }

    private DrillHeadPerm getHeadPerm(ItemStack stack) {
        if (stack.getItemDamage() >= 0 && stack.getItemDamage() < perms.length) return perms[stack.getItemDamage()];
    ...
    public void registerIcons(IIconRegister ir) {
        ...
        for (DrillHeadPerm p : this.perms) p.icon = ir.registerIcon(p.texture);
    }
