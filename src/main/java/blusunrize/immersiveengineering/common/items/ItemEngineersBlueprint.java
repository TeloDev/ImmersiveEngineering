...
public class ItemEngineersBlueprint extends ItemUpgradeableTool {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean adv) {
        ...
        if (stack.getItemDamage() < sub.length) {
            ...
            if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard.isKeyDown(Keyboard.KEY_RSHIFT)) {
                ...
                if (recipes.length > 0)
                    for (int i = 0; i < recipes.length; i++) list.add(" " + recipes[i].output.getDisplayName());
            } else list.add(StatCollector.translateToLocal(Lib.DESC_INFO + "blueprint.creates0"));
        }
    }
    ...
    public boolean canTakeFromWorkbench(ItemStack stack) {
        ItemStack[] stored = this.getContainedItems(stack);
        for (int i = 0; i < 6; i++) if (stored[i] != null) return false;
        return true;
    }
}
