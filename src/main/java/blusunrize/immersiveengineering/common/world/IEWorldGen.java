...
public class IEWorldGen implements IWorldGenerator {
    ...
    public static OreGen addOreGen(
            Block block, int meta, int maxVeinSize, int minY, int maxY, int chunkOccurence, int weight) {
    ...
    public void generate(
        ...
        if (!oreDimBlacklist.contains(world.provider.dimensionId))
            for (OreGen gen : orespawnList) gen.generate(world, random, chunkX * 16, chunkZ * 16);
    }
}
