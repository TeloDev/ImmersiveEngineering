...
public class EntityRevolvershotHoming extends EntityRevolvershot {
    ...
    public EntityRevolvershotHoming(
            World world, double x, double y, double z, double ax, double ay, double az, int type) {
        ...
        if (!worldObj.isRemote && this.ticksExisted > trackCountdown) {
            ...
            if (target != null) {
                Vec3 newMotion = Vec3.createVectorHelper(
                                ...
                                motionY * (1 - redirectionSpeed)
                                        + ((target.posY + target.height / 2) - this.posY) * redirectionSpeed,
                                motionZ * (1 - redirectionSpeed) + (target.posZ - this.posZ) * redirectionSpeed)
                        .normalize();
    ...
    public EntityLivingBase getTarget() {
        if (targetOverride != null && !targetOverride.isDead) return targetOverride;
        double r = 20D;
        ...
        for (Object o : worldObj.getEntitiesWithinAABB(EntityLivingBase.class, aabb))
            if (o instanceof EntityLivingBase && !o.equals(this.shootingEntity))
                if (target == null
                        || ((EntityLivingBase) o).getDistanceSqToEntity(this) < target.getDistanceSqToEntity(this))
