...
public abstract class IESlot extends Slot {
    ...
    public static class Output extends IESlot {
        ...
        public void onPickupFromSlot(EntityPlayer player, ItemStack stack) {
            ...
            if (player != null
                    && (this.container instanceof ContainerBlastFurnace
                            || this.container instanceof ContainerArcFurnace)
                    && OreDictionary.itemMatches(new ItemStack(IEContent.itemMetal, 1, 7), stack, true))
    ...
    public static class FluidContainer extends IESlot {
        ...
        @Override
        public boolean isItemValid(ItemStack itemStack) {
            if (empty) return FluidContainerRegistry.isEmptyContainer(itemStack);
            else return FluidContainerRegistry.isFilledContainer(itemStack);
        }
    }
            ...
            if (preventDoubles)
                for (Slot slot : (List<Slot>) container.inventorySlots)
                    if (slot instanceof IESlot.Upgrades
                            && ((IESlot.Upgrades) slot).preventDoubles
                            && OreDictionary.itemMatches(slot.getStack(), itemStack, true)) return false;
            return itemStack != null
                    && itemStack.getItem() instanceof IUpgrade
                    && ((IUpgrade) itemStack.getItem())
                            .getUpgradeTypes(itemStack)
                            .contains(type)
                    && ((IUpgrade) itemStack.getItem()).canApplyUpgrades(upgradeableTool, itemStack);
        }
    ...
    public static class Shader extends IESlot {
        ...
        public boolean isItemValid(ItemStack itemStack) {
            if (itemStack == null
                    || !(itemStack.getItem() instanceof IShaderItem)
                    || tool == null
                    || !(tool.getItem() instanceof IShaderEquipableItem)) return false;
            return ((IShaderItem) itemStack.getItem())
                            .getShaderCase(itemStack, tool, ((IShaderEquipableItem) tool.getItem()).getShaderType())
                    != null;
        }
    ...
    public static class UpgradeableItem extends IESlot {
        ...
        public boolean isItemValid(ItemStack itemStack) {
            return itemStack != null
                    && itemStack.getItem() instanceof IUpgradeableTool
                    && ((IUpgradeableTool) itemStack.getItem()).canModify(itemStack);
        }
        ...
        public void onSlotChanged() {
            super.onSlotChanged();
            if (container instanceof ContainerModWorkbench) ((ContainerModWorkbench) container).rebindSlots();
        }

        @Override
        public boolean canTakeStack(EntityPlayer player) {
            if (this.getStack() != null
                    && getStack().getItem() instanceof IUpgradeableTool
                    && !((IUpgradeableTool) getStack().getItem()).canTakeFromWorkbench(getStack())) return false;
            return true;
        }
    ...
    public static class BlueprintOutput extends IESlot {
        ...
        public void onPickupFromSlot(EntityPlayer player, ItemStack stack) {
            if (upgradeableTool != null && upgradeableTool.getItem() instanceof ItemEngineersBlueprint)
                ((ItemEngineersBlueprint) upgradeableTool.getItem())
                        .reduceInputs(recipe, upgradeableTool, stack, this.container);
