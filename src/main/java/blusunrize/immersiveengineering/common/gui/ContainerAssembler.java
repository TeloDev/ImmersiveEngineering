...
public class ContainerAssembler extends Container {
    ...
    public ItemStack slotClick(int id, int button, int modifier, EntityPlayer player) {
        ...
        if (!(slot instanceof IESlot.Ghost)) return super.slotClick(id, button, modifier, player);
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slot) {
        ...
        if (slotObject != null && slotObject.getHasStack() && !(slotObject instanceof IESlot.Ghost)) {
            ...
            if (slot < 48) {
                if (!this.mergeItemStack(stackInSlot, 48, (48 + 36), true)) return null;
            } else {
                if (!this.mergeItemStack(stackInSlot, 30, 48, false)) return null;
            }
