...
public class ItemBlockWoodenDecoration extends ItemBlockIEBase {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean advInfo) {
        if (stack.getItemDamage() == 0) list.add("This item is deprecated. Hold it in your inventory to update it.");
    }
            ...
            if (meta == 0)
                ((EntityPlayer) ent)
                        .inventory.setInventorySlotContents(
                                slot, new ItemStack(IEContent.blockTreatedWood, stack.stackSize));
        }
    }
    ...
    public boolean onItemUse(
            ...
            float hitY,
            float hitZ) {
        if (stack.getItemDamage() != 2) return super.onItemUse(stack, player, world, x, y, z, side, hitX, hitY, hitZ);
        ...
        if (this.field_150939_a.equals(world.getBlock(x1, y1, z1))
        ...
        if (side == 0 || (side != 1 && hitY >= .5)) meta = 3;
    ...
    public boolean placeBlockAt(
        ...
        if (ret) {
            ...
            if (tileEntity instanceof TileEntityWallmount) {
                ...
                if (side < 2) ((TileEntityWallmount) tileEntity).sideAttached = side + 1;
            }
        }
        return ret;
    }
