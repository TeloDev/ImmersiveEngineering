...
public class ItemBlockWoodenDevices extends ItemBlockIEBase {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean advInfo) {
        if (stack.getItemDamage() == 4) list.add(StatCollector.translateToLocal(Lib.DESC_FLAVOUR + "crate"));
    ...
    public boolean placeBlockAt(
        ...
        if (meta >= 1 && meta <= 3) f = side;

        if (meta == 0) {
            for (int i = 0; i <= 3; i++)
                if (!player.canPlayerEdit(x, y + 1, z, side, stack)
                        || !world.getBlock(x, y + i, z).isReplaceable(world, x, y + i, z)) return false;
        } else if (meta == 1)
            for (int yy = -2; yy <= 2; yy++) {
                int r = yy < -1 || yy > 1 ? 1 : 2;
                for (int ww = -r; ww <= r; ww++)
                    if (!world.getBlock(x + ((f == 2 || f == 3) ? ww : 0), y + yy, z + ((f == 2 || f == 3) ? 0 : ww))
                            .isReplaceable(
                                    world,
                                    x + ((f == 2 || f == 3) ? ww : 0),
                                    y + yy,
                                    z + ((f == 2 || f == 3) ? 0 : ww))) return false;
            }
        else if (meta == 2 || meta == 3)
            for (int yy = -6; yy <= 6; yy++) {
                int r = Math.abs(yy) == 6
                        ? 1
                        : Math.abs(yy) == 5 ? 3 : Math.abs(yy) == 4 ? 4 : Math.abs(yy) > 1 ? 5 : 6;
                for (int ww = -r; ww <= r; ww++)
                    if (!world.getBlock(x + (f <= 3 ? ww : 0), y + yy, z + (f <= 3 ? 0 : ww))
                            .isReplaceable(world, x + (f <= 3 ? ww : 0), y + yy, z + (f <= 3 ? 0 : ww))) return false;
            }
        else if (meta == 5) {
            if (f == 2 || f == 3) {
                if (!world.getBlock(x + 1, y, z).isReplaceable(world, x + 1, y, z)
                        && !world.getBlock(x - 1, y, 1).isReplaceable(world, x - 1, y, z)) return false;
            } else {
                if (!world.getBlock(x, y, z + 1).isReplaceable(world, x, y, z + 1)
                        && !world.getBlock(x, y, z - 1).isReplaceable(world, x, y, z - 1)) return false;
            }
        }
        ...
        if (tileEntity instanceof TileEntityWoodenPost) {
                ...
                for (int ww = -r; ww <= r; ww++)
                    if (yy != 0 || ww != 0) {
                        world.setBlock(
                                ...
                                meta,
                                0x3);
                        tileEntityWatermill = world.getTileEntity(
                                x + ((f == 2 || f == 3) ? ww : 0), y + yy, z + ((f == 2 || f == 3) ? 0 : ww));
            ...
            if (stack.hasTagCompound()) ((TileEntityWoodenBarrel) tileEntity).readTank(stack.getTagCompound());
        }
        return true;
    }
}
