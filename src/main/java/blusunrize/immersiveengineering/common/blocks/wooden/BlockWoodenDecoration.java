...
public class BlockWoodenDecoration extends BlockIEBase // implements blusunrize.aquatweaks.api.IAquaConnectable
{
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        if (metadata == 4) ret.add(new ItemStack(this, 1, 2));
        return ret;
    }
    ...
    public boolean isSideSolid(IBlockAccess world, int x, int y, int z, ForgeDirection side) {
        ...
        if (meta == 1) return side == UP;
        if (meta == 2) return side == DOWN;
        if (meta == 3) return side == UP;
    ...
    public void getSubBlocks(Item item, CreativeTabs tab, List list) {
        for (int i = 0; i < subNames.length; i++) if (i != 0 && i != 3) list.add(new ItemStack(item, 1, i));
    }
