        ...
        implements IPostBlock // , blusunrize.aquatweaks.api.IAquaConnectable
{
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        for (int i = 0; i < this.iconDimensions; i++) {
            ...
            else icons[4][i] = iconRegister.registerIcon("immersiveengineering:woodenCrate");
    ...
    public IIcon getIcon(int side, int meta) {
        if (meta == 6 && side < 2) return iconBarrel[0];
    ...
    boolean isPost(IBlockAccess world, int x, int y, int z, int type) {
        TileEntity te = world.getTileEntity(x, y, z);
        if (te instanceof TileEntityWoodenPost) return ((TileEntityWoodenPost) te).type == type;
    ...
    public boolean onBlockActivated(
        ...
        if (te instanceof TileEntityWindmillAdvanced
            ...
            if (((TileEntityWindmillAdvanced) te).dye[sel] == heldDye) return false;
        ...
        if (!player.isSneaking() && te instanceof TileEntityWoodenCrate) {
            if (!world.isRemote) player.openGui(ImmersiveEngineering.instance, Lib.GUIID_WoodenCrate, world, x, y, z);
            return true;
        }
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        if (metadata == 0 || metadata == 4 || metadata == 6) return ret;
    ...
    public void breakBlock(World world, int x, int y, int z, Block par5, int par6) {
        ...
        if (tileEntity instanceof TileEntityWatermill) {
            ...
            if (!(off[0] == 0 && off[1] == 0) && world.isAirBlock(xx, yy, zz)) return;
