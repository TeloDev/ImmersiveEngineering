...
public class MultiblockLightningRod implements IMultiblock {
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        for (int xx = -1; xx <= 1; xx++)
            for (int zz = -1; zz <= 1; zz++) {
                ...
                if (!(tes[xx + 1][zz + 1] instanceof TileEntityLightningRod)
                        || ((TileEntityLightningRod) tes[xx + 1][zz + 1]).formed) {
                    return false;
                }
            }
        for (int xx = -1; xx <= 1; xx++)
    ...
    public ItemStack[] getTotalMaterials() {
        return new ItemStack[] {
            new ItemStack(IEContent.blockMetalMultiblocks, 9, BlockMetalMultiblocks.META_lightningRod)
        };
    }
}
