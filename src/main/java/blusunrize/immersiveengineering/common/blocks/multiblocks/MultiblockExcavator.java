...
public class MultiblockExcavator implements IMultiblock {
                    ...
                    if (l == 5) {
                        if (w != 0 && (h != 1 || w != 1))
                            ...
                            m = h == 1 && w == 0
                                    ...
                                    : BlockMetalDecoration.META_heavyEngineering;
                    } else if (l == 4) {
                        if (w == 0 && h == 1) structure[h][l][w] = new ItemStack(IEContent.blockStorage, 1, 7);
                        else
                            m = w == 2
                                    ? BlockMetalDecoration.META_lightEngineering
                                    : BlockMetalDecoration.META_heavyEngineering;
                    } else if (l == 3) structure[h][l][w] = new ItemStack(IEContent.blockStorage, 1, 7);
                    else if (w == 2)
                        ...
                        m = h == 0
                                ? BlockMetalDecoration.META_heavyEngineering
                                : BlockMetalDecoration.META_lightEngineering;
                    else m = BlockMetalDecoration.META_lightEngineering;
                    if (m >= 0) structure[h][l][w] = new ItemStack(IEContent.blockMetalDecoration, 1, m);
                }
    }
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        //		if(world.getBlock(x,y-1,z).equals(IEContent.blockMetalDecoration) &&
        // world.getBlockMetadata(x,y-1,z)==BlockMetalDecoration.META_scaffolding
        //				&&
        ...
        if (b) {
            for (int l = 0; l < 6; l++)
                for (int w = -1; w <= 1; w++)
                    for (int h = -1; h <= 1; h++) {
                        ...
                        world.setBlock(
                                xx, yy, zz, IEContent.blockMetalMultiblocks, BlockMetalMultiblocks.META_excavator, 0x3);
            ...
            if (MultiblockBucketWheel.instance.isBlockTrigger(
                    world.getBlock(wheelX, startY, wheelZ), world.getBlockMetadata(wheelX, startY, wheelZ)))
                MultiblockBucketWheel.instance.createStructure(
                        world, wheelX, startY, wheelZ, ForgeDirection.ROTATION_MATRIX[0][side], player);
        }
        return b;
    }
            ...
            for (int w = -1; w <= 1; w++)
                for (int h = -1; h <= 1; h++) {
                    ...
                    if (OreDictionary.itemMatches(
                            structure[h + 1][5 - l][w + 1], new ItemStack(IEContent.blockStorage, 1, 7), true)) {
                        // Steelblocks should have OreDict checks
                        if (!Utils.compareToOreName(checkStack, "blockSteel")) return false;
                    } else if (!OreDictionary.itemMatches(structure[h + 1][5 - l][w + 1], checkStack, true))
                        return false;
                }
        return true;
    }
