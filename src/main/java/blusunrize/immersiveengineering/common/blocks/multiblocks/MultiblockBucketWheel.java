        ...
        for (int h = 0; h < 7; h++)
            for (int l = 0; l < 7; l++) {
                ...
                if (l == 0
                        ...
                        || h == 6
                        || ((l == 1 || l == 5) && (h == 1 || h == 5))
                        || (h == 3 && l == 3)) structure[h][l][0] = new ItemStack(IEContent.blockStorage, 1, 7);
                else
                    structure[h][l][0] =
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        for (int h = -3; h <= 3; h++)
            for (int w = -3; w <= 3; w++) {
                ...
                if ((h == -3 || h == 3) && w != 0) continue;
                if ((w == -3 || w == 3) && h != 0) continue;
                if (w == -3
                        || h == -3
                        || w == 3
                        || h == 3
        ...
        for (int h = -3; h <= 3; h++)
            for (int w = -3; w <= 3; w++) {
                ...
                world.setBlock(
                        xx, yy, zz, IEContent.blockMetalMultiblocks, BlockMetalMultiblocks.META_bucketWheel, 0x3);
                ...
                if (curr instanceof TileEntityBucketWheel) {
                    ...
                    tile.pos = (w + 3) + (h + 3) * 7;
                    tile.offset =
                            new int[] {(side == 2 ? w : side == 3 ? -w : 0), h, (side == 4 ? w : side == 5 ? -w : 0)};
                }
            }
        return true;
    }
