        ...
        for (int w = 0; w < 5; w++)
            for (int l = 0; l < 3; l++)
                for (int h = 0; h < 3; h++) {
                    ...
                    if (h == 0) {
                        if (l != 1 && w != 2) m = BlockMetalDecoration.META_scaffolding;
                        else if (w > 0 && w < 4) m = BlockMetalDecoration.META_lightEngineering;
                    } else if (h == 1 && w == 2 && l == 2) m = BlockMetalDecoration.META_lightEngineering;
                    else if (h > 0 && w != 2) m = BlockMetalDecoration.META_sheetMetal;
        ...
        for (int l = 0; l < 3; l++)
            for (int w = -2; w <= 2; w++)
                for (int h = -1; h <= 1; h++) {
                    ...
                    int m = BlockMetalDecoration.META_heavyEngineering;
                    if (h == -1) {
                        if (l != 1 && w != 0) m = BlockMetalDecoration.META_scaffolding;
                        else if (w > -2 && w < 2) m = BlockMetalDecoration.META_lightEngineering;
                    } else if (h == 0 && w == 0 && l == 2) m = BlockMetalDecoration.META_lightEngineering;
                    else if (h > -1 && w != 0) m = BlockMetalDecoration.META_sheetMetal;
                    ...
                    if (!(world.getBlock(xx, yy, zz).equals(IEContent.blockMetalDecoration)
                            && world.getBlockMetadata(xx, yy, zz) == m)) return false;
                }
            ...
            for (int w = -2; w <= 2; w++)
                for (int h = -1; h <= 1; h++) {
                    ...
                    world.setBlock(
                            ...
                            BlockMetalMultiblocks.META_refinery,
                            0x3);
