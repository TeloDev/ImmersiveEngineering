...
public class MultiblockBottlingMachine implements IMultiblock {
    ...
    public boolean overwriteBlockRender(ItemStack stack, int iterator) {
        if (iterator == 6 || iterator == 9) GL11.glRotatef(90, 0, 1, 0);
        if (iterator == 8) GL11.glRotatef(-90, 0, 1, 0);
        return false;
    }
                ...
                for (int l = 0; l < 2; l++)
                    for (int w = -1; w <= 1; w++) {
                        ...
                        world.setBlock(
                                ...
                                BlockMetalMultiblocks.META_bottlingMachine,
                                0x3);
            ...
            for (int l = 0; l < 2; l++)
                for (int w = -1; w <= 1; w++)
                    if (structure[h + 1][1 - l][w + 1] != null) {
                        ...
                        if (!OreDictionary.itemMatches(structure[h + 1][1 - l][w + 1], checkStack, true)) return false;
                        ...
                        if (tile instanceof TileEntityConveyorBelt) {
                            ...
                            int fExpected = l == 1
                                    ? (w == -1 ? ForgeDirection.OPPOSITES[side] : side)
                                    : w < 1 ? ForgeDirection.ROTATION_MATRIX[mirror ? 0 : 1][side] : side;
                            if (f != fExpected) return false;
                        }
                    }
        return true;
    }
