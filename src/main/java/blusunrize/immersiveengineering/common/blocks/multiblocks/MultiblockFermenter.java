...
public class MultiblockFermenter implements IMultiblock {
        ...
        for (int l = 0; l < 3; l++)
            for (int w = -1; w <= 1; w++)
                for (int h = -1; h <= 1; h++) {
                    ...
                    if (h == 0 && (w != 0 || l != 1)) {
                        if (!(world.getBlock(xx, yy, zz).equals(IEContent.blockMetalMultiblocks)
                                && world.getBlockMetadata(xx, yy, zz) == BlockMetalMultiblocks.META_fermenter))
                            return false;
                    } else {
        ...
        for (int l = 0; l < 3; l++)
            for (int w = -1; w <= 1; w++)
                for (int h = -1; h <= 1; h++) {
                    ...
                    if (h == 0 && (w != 0 || l != 1)) world.markBlockForUpdate(startX + xx, startY + yy, startZ + zz);
                    else
                        world.setBlock(
                                startX + xx,
                                startY + yy,
                                ...
                                BlockMetalMultiblocks.META_fermenter,
                                0x3);
    ...
    public ItemStack[] getTotalMaterials() {
        return new ItemStack[] {
            new ItemStack(IEContent.blockMetalDecoration, 19, BlockMetalDecoration.META_lightEngineering),
            new ItemStack(IEContent.blockMetalMultiblocks, 8, BlockMetalMultiblocks.META_fermenter)
        };
    }
}
