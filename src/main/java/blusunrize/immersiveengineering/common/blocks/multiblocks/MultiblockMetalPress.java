...
public class MultiblockMetalPress implements IMultiblock {
        ...
        for (int h = 0; h < 3; h++)
            for (int l = 0; l < 3; l++)
                if (h == 0)
                    ...
                    if (l == 1) structure[h][l][0] = new ItemStack(Blocks.piston, 1, 0);
                    else
                        structure[h][l][0] =
                                new ItemStack(IEContent.blockMetalDevice, 1, BlockMetalDevices.META_conveyorBelt);
                } else if (h == 2 && l == 1)
                    structure[h][l][0] = new ItemStack(
                            IEContent.blockMetalDecoration, 1, BlockMetalDecoration.META_heavyEngineering);
    }
    ...
    public boolean overwriteBlockRender(ItemStack stack, int iterator) {
        if (iterator == 4) GL11.glRotatef(-180, 1, 0, 0);
        if (iterator == 3 || iterator == 5) GL11.glRotatef(90, 0, 1, 0);
        return false;
    }
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        if (world.getTileEntity(
                        startX + (dir == 4 ? -1 : dir == 5 ? 1 : 0),
                        startY,
                        startZ + (dir == 2 ? -1 : dir == 3 ? 1 : 0))
                instanceof TileEntityConveyorBelt)
            dir = ForgeDirection.OPPOSITES[
                    ((TileEntityConveyorBelt) world.getTileEntity(
                                    startX + (dir == 4 ? -1 : dir == 5 ? 1 : 0),
                                    startY,
                                    startZ + (dir == 2 ? -1 : dir == 3 ? 1 : 0)))
                            .facing];
        ...
        for (int l = -1; l <= 1; l++)
            for (int h = -1; h <= 1; h++) {
                ...
                if (h == -1) {
                    ...
                    if (l == 0) {
                        if (!(world.getBlock(xx, yy, zz).equals(Blocks.piston)
                                && world.getBlockMetadata(xx, yy, zz) == 0)) return false;
                    } else {
                        if (!(world.getBlock(xx, yy, zz).equals(IEContent.blockMetalDevice)
                            ...
                            return false;
                        if (((TileEntityConveyorBelt) world.getTileEntity(xx, yy, zz)).facing
                                != ForgeDirection.OPPOSITES[dir]) return false;
                    }
                } else {
                    if (!(world.getBlock(xx, yy, zz).equals(IEContent.blockMetalDecoration)
                            && world.getBlockMetadata(xx, yy, zz) == BlockMetalDecoration.META_heavyEngineering))
                        return false;
                }
            }
