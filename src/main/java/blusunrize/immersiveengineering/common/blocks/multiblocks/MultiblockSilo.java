...
public class MultiblockSilo implements IMultiblock {
                ...
                for (int zz = -1; zz <= 1; zz++)
                    if (yy == 0) {
                        if (Math.abs(xx) == 1 && Math.abs(zz) == 1) {
                            if (!world.getBlock(startX + xx, startY + yy, startZ + zz)
                                            .equals(IEContent.blockWoodenDecoration)
                                    || world.getBlockMetadata(startX + xx, startY + yy, startZ + zz) != 1) return false;
                        } else if (xx == 0 && zz == 0)
                            if (!world.getBlock(startX + xx, startY + yy, startZ + zz)
                                            .equals(IEContent.blockMetalDecoration)
                                    || world.getBlockMetadata(startX + xx, startY + yy, startZ + zz)
                                            != BlockMetalDecoration.META_sheetMetal) return false;
                    } else {
                        if (yy > 0 && yy < 6 && xx == 0 && zz == 0) {
                            if (!world.isAirBlock(startX + xx, startY + yy, startZ + zz)) return false;
                        } else if (!world.getBlock(startX + xx, startY + yy, startZ + zz)
                                        .equals(IEContent.blockMetalDecoration)
                                || world.getBlockMetadata(startX + xx, startY + yy, startZ + zz)
                                        != BlockMetalDecoration.META_sheetMetal) return false;
                    }
            ...
            for (int xx = -1; xx <= 1; xx++)
                for (int zz = -1; zz <= 1; zz++)
                    if (yy > 0 || (Math.abs(xx) == 1 && Math.abs(zz) == 1) || (xx == 0 && zz == 0)) {
                        if (yy > 0 && yy < 6 && xx == 0 && zz == 0) continue;
                        world.setBlock(
                                startX + xx,
                                startY + yy,
                                startZ + zz,
                                IEContent.blockMetalMultiblocks,
                                BlockMetalMultiblocks.META_silo,
                                0x3);
    ...
    public ItemStack[] getTotalMaterials() {
        return new ItemStack[] {
            new ItemStack(IEContent.blockWoodenDecoration, 4, 1),
            new ItemStack(IEContent.blockMetalDecoration, 50, BlockMetalDecoration.META_sheetMetal)
        };
    }
}
