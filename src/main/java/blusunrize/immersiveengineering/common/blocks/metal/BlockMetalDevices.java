...
public class BlockMetalDevices extends BlockIEBase // implements blusunrize.aquatweaks.api.IAquaConnectable
{
    ...
    public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
        ...
        if (te instanceof TileEntityCapacitorLV) {
            ...
            if (side == 0) return icon_capacitorBot[t][cap.sideConfig[side] + 1];
            else if (side == 1) return icon_capacitorTop[t][cap.sideConfig[side] + 1];
            else return icon_capacitorSide[t][cap.sideConfig[side] + 1];
        }
        ...
        if (te instanceof TileEntityFurnaceHeater) {
            if (((TileEntityFurnaceHeater) te).sockets[side] == 1) return icons[META_furnaceHeater][0];
            else return icons[META_furnaceHeater][((TileEntityFurnaceHeater) te).showActiveTexture() ? 2 : 1];
        }
        if (world.getBlockMetadata(x, y, z) == META_sorter) return icons_sorter[side];
    ...
    public IIcon getIcon(int side, int meta) {
        if (meta == META_sorter) return icons_sorter[side];
    ...
    public boolean onBlockActivated(
        ...
        if (te instanceof TileEntityConveyorBelt && Utils.isHammer(player.getCurrentEquippedItem())) {
            if (!world.isRemote) {
                ...
                } else tile.facing = ForgeDirection.ROTATION_MATRIX[1][tile.facing];
                world.markBlockForUpdate(x, y, z);
            }
            return true;
        }
    ...
    public boolean isSideSolid(IBlockAccess world, int x, int y, int z, ForgeDirection side) {
        ...
        if (meta == META_capacitorLV || meta == META_capacitorMV || meta == META_capacitorHV) return true;
        if (meta == META_dynamo || meta == META_thermoelectricGen || meta == META_furnaceHeater) return true;
        if (meta == META_sampleDrill) {
            TileEntity te = world.getTileEntity(x, y, z);
            if (te instanceof TileEntitySampleDrill && ((TileEntitySampleDrill) te).pos == 0) return true;
        }
        return false;
    }
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity par5Entity) {
        ...
        if (par5Entity != null
            ...
            if (tile.transportUp) vY = 0.17D * vBase;
            else if (tile.transportDown) vY = -0.07000000000000001D * vBase;
    ...
    public boolean shouldRenderFluid(IBlockAccess world, int x, int y, int z) {
        int meta = world.getBlockMetadata(x, y, z);
        return meta == META_connectorLV || meta == META_connectorMV || meta == META_relayHV || meta == META_connectorHV;
    }
