...
public class TileEntityBottlingMachine extends TileEntityMultiblockPart
    ...
    public float[] getBlockBounds() {
        ...
        if ((pos % 3 == 0 && ff == 4) || (pos % 3 == 2 && ff == 5)) zMin = .4375f;
        else if ((pos % 3 == 0 && ff == 5) || (pos % 3 == 2 && ff == 4)) zMax = .5625f;
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        ...
        TileEntityBottlingMachine master = master();
        if (master != null && canFill(from, resource.getFluid())) return master.fill(from, resource, doFill);
    ...
    public int getEmptyCannister(int id) throws IllegalArgumentException {
        ...
        while (id >= 0) {
            ...
            if (min > 72) throw new IllegalArgumentException("Not enough empty cannisters found");
            currId = minId;
            currVal = min;
            id--;
        }
    ...
    public int getEmptyCount() {
        int count = 0;
        for (int i = 0; i < 5; i++) if (process[i] > -1 && process[i] <= 72) count++;
        return count;
    }
    ...
    public int getFilledCannister(int id) throws IllegalArgumentException {
        ...
        while (id >= 0) {
            ...
            if (minId < 0) throw new IllegalArgumentException("Not enough filled cannisters found");
            currId = minId;
            currVal = min;
            id--;
        }
    ...
    public int getFilledCount() {
        int count = 0;
        for (int i = 0; i < 5; i++) if (process[i] > 72) count++;
        return count;
    }
}
