...
public class TileEntitySampleDrill extends TileEntityIEBase implements IEnergyReceiver {
    ...
    public String getVeinLocalizedName() {
        ...
        if (unlocalizedName.equals(localizedName)) return name;
    ...
    public int getEnergyStored(ForgeDirection from) {
        if (pos != 0) {
            TileEntity te = worldObj.getTileEntity(xCoord, yCoord - pos, zCoord);
            if (te instanceof TileEntitySampleDrill) return ((TileEntitySampleDrill) te).getEnergyStored(from);
        }
    ...
    public int getMaxEnergyStored(ForgeDirection from) {
        if (pos != 0) {
            TileEntity te = worldObj.getTileEntity(xCoord, yCoord - pos, zCoord);
            if (te instanceof TileEntitySampleDrill) return ((TileEntitySampleDrill) te).getMaxEnergyStored(from);
        }
