...
public class TileEntityEnergyMeter extends TileEntityImmersiveConnectable {
    ...
    public void updateEntity() {
        if (dummy || worldObj.isRemote) return;
        synchronized (lastPackets) {
            // Yes, this might tick in between different connectors sending power, but since this is a block for
            // statistical evaluation over a tick, that is irrelevant.
    ...
    private void updateComparatorValues() {
        if (maxTransfer < 0) {
            ...
            maxTransfer = 0;
            for (Connection c : conns) maxTransfer += c.cableType.getTransferRate();
        }
        ...
        if (oldComp != compVal) {
            ...
            if (!(te instanceof TileEntityEnergyMeter)) return;
    ...
    public void connectCable(WireType cableType, TargetingInfo target) {
        if (dummy) {
            TileEntity above = worldObj.getTileEntity(xCoord, yCoord + 1, zCoord);
            if (above instanceof TileEntityEnergyMeter) ((TileEntityEnergyMeter) above).connectCable(cableType, target);
        } else {
            super.connectCable(cableType, target);
            if (!worldObj.isRemote) maxTransfer += cableType.getTransferRate();
        }
    }
    ...
    public void removeCable(Connection connection) {
        ...
        if (!worldObj.isRemote) {
            if (connection == null) maxTransfer = 0;
            else maxTransfer -= connection.cableType.getTransferRate();
        }
    }
    ...
    public double getAveragePower() {
        ...
        if (te.dummy) {
            ...
            if (!(tmp instanceof TileEntityEnergyMeter)) return -1;
            te = (TileEntityEnergyMeter) tmp;
        }
        if (te.lastPackets.size() == 0) return 0;
        double sum = 0;
        synchronized (te.lastPackets) {
            for (double transfer : te.lastPackets) sum += transfer;
        }
