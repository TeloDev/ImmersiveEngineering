...
public class TileEntityChargingStation extends TileEntityIEBase implements IEnergyReceiver, IInventory {
    ...
    public void updateEntity() {
        if (inventory != null
                && (inventory.getItem() instanceof IEnergyContainerItem
                        || (Lib.IC2 && IC2Helper.isElectricItem(inventory)))) {
            if (worldObj.isRemote && charging) {
                ...
                if (inventory.getItem() instanceof IEnergyContainerItem) {
                    ...
                    if (max > 0) charge = container.getEnergyStored(inventory) / (float) max;
                } else {
                    double max = IC2Helper.getMaxItemCharge(inventory);
                    if (max > 0) charge = (float) (IC2Helper.getCurrentItemCharge(inventory) / max);
                }
                ...
                for (int i = 0; i < max; i++) {
                    ...
                    if (charge >= 1 || (time % 12 >= i * 4 && time % 12 <= i * 4 + 2)) {
                        ...
                        double x = xCoord
                                ...
                                + (facing == 4
                                        ? -.46875
                                        : facing == 5 ? .46875 : facing == 2 ? (-.1875 * shift) : (.1875 * shift));
                        double y = yCoord + .25;
                        double z = zCoord
                                + .5
                                + (facing == 2
                                        ? -.46875
                                        : facing == 3 ? .46875 : facing == 5 ? (-.1875 * shift) : (.1875 * shift));
                        ImmersiveEngineering.proxy.spawnRedstoneFX(
                                worldObj, x, y, z, .25, .25, .25, .5f, 1 - charge, charge, 0);
                    }
                }
            } else if (charging) {
                ...
                if (inventory.getItem() instanceof IEnergyContainerItem) {
                    ...
                    if (max > 0 && space > 0) {
                        ...
                        int accepted = Math.min(
                                container.receiveEnergy(inventory, insert, true),
                                this.energyStorage.extractEnergy(insert, true));
                        ...
                        if (energyDec != energyDecNew) worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
                    }
                } else {
                    double max = IC2Helper.getMaxItemCharge(inventory);
                    ...
                    if (max > 0 && space > 0) {
                        ...
                        double insert = Math.min(
                                space, ModCompatability.convertRFtoEU(Config.getInt("charger_consumption"), 5));
                        double accepted = IC2Helper.chargeItem(inventory, insert, true);
                        int rfAccepted =
                                this.energyStorage.extractEnergy(ModCompatability.convertEUtoRF(accepted), false);
                        if (rfAccepted > 0)
        ...
        if (!worldObj.isRemote && worldObj.getTotalWorldTime() % 32 == ((xCoord ^ zCoord) & 31)) {
            ...
            if (inventory != null
                ...
                if (inventory.getItem() instanceof IEnergyContainerItem) {
                    ...
                    charge =
                            (float) (IC2Helper.getCurrentItemCharge(inventory) / IC2Helper.getMaxItemCharge(inventory));
            int i = (int) (15 * charge);
    ...
    public void writeCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        if (inventory != null) nbt.setTag("inventory", inventory.writeToNBT(new NBTTagCompound()));
    }
    ...
    public boolean canConnectEnergy(ForgeDirection from) {
        return from == ForgeDirection.DOWN
                || (from != null && from.getOpposite().ordinal() == facing);
    }
