...
public class TileEntityTransformer extends TileEntityImmersiveConnectable {
    ...
    public void writeCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        if (secondCable != null) nbt.setString("secondCable", secondCable.getUniqueName());
    ...
    public void readCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        if (nbt.hasKey("secondCable")) secondCable = ApiUtils.getWireTypeFromNBT(nbt, "secondCable");
    ...
    public void connectCable(WireType cableType, TargetingInfo target) {
        if (dummy) {
            TileEntity above = worldObj.getTileEntity(xCoord, yCoord + 1, zCoord);
            if (above instanceof TileEntityTransformer) ((TileEntityTransformer) above).connectCable(cableType, target);
            else return;
        }
        switch (getTargetedConnector(target)) {
            case 0:
                ...
                break;
            case 1:
                if (secondCable == null) this.secondCable = cableType;
                break;
        }
    }
    ...
    public Vec3 getRaytraceOffset(IImmersiveConnectable link) {
        if (postAttached > 0) return Vec3.createVectorHelper(.5, 1.5, .5);
    ...
    public Vec3 getConnectionOffset(Connection con) {
        ...
        if (postAttached > 0) {
            ...
            if (facing == 2) return Vec3.createVectorHelper(b ? .8125 : .1875, 1.5 - conRadius, .5);
            if (facing == 3) return Vec3.createVectorHelper(b ? .1875 : .8125, 1.5 - conRadius, .5);
            if (facing == 4) return Vec3.createVectorHelper(.5, 1.5 - conRadius, b ? .1875 : .8125);
            if (facing == 5) return Vec3.createVectorHelper(.5, 1.5 - conRadius, b ? .8125 : .1875);
        }
