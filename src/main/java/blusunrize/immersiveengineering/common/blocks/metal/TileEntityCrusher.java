...
public class TileEntityCrusher extends TileEntityMultiblockPart
    ...
    public void updateEntity() {
        ...
        if (hasPower && (active && process > 0) || mobGrinding || grindingTimer > 0) {
            if (grindingTimer > 0) grindingTimer--;
            barrelRotation += 18f;
            barrelRotation %= 360f;
        }
    ...
    public void readCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        if (hasPower) barrelRotation = nbt.getFloat("barrelRotation");
    ...
    public void setInventorySlotContents(int slot, ItemStack stack) {
        if (!formed || pos != 27) return;
        TileEntityCrusher master = master();
        if (master != null) master.addStackToInputs(stack);
    }
