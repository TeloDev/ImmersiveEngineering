...
public class TileEntityBucketWheel extends TileEntityMultiblockPart {
    ...
    public void readCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        rotation = (Math.abs(nbtRot - rotation) > 5 * (float) Config.getDouble("excavator_speed"))
                ? nbtRot
                : rotation; // avoid stuttering due to packet delays
    ...
    public void writeCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        if (particleStack != null) nbt.setTag("particleStack", particleStack.writeToNBT(new NBTTagCompound()));
    }

    @Override
    public void updateEntity() {
        if (!formed || pos != 24) return;
    ...
    public float[] getBlockBounds() {
        if (pos == 3 || pos == 9 || pos == 11) return new float[] {0, .25f, 0, 1, 1, 1};
        else if (pos == 45 || pos == 37 || pos == 39) return new float[] {0, 0, 0, 1, .75f, 1};
        else if (pos == 21)
            return new float[] {
                facing == 2 ? .25f : 0, 0, facing == 4 ? .25f : 0, facing == 3 ? .75f : 1, 1, facing == 5 ? .75f : 1
            };
        else if (pos == 27)
            return new float[] {
                facing == 3 ? .25f : 0, 0, facing == 5 ? .25f : 0, facing == 2 ? .75f : 1, 1, facing == 4 ? .75f : 1
            };
