    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean advInfo) {
        if (((BlockIEBase) field_150939_a).subNames[stack.getItemDamage()].startsWith("capacitor")) {
            list.add(StatCollector.translateToLocalFormatted(
                    "desc.ImmersiveEngineering.info.energyStored", ItemNBTHelper.getInt(stack, "energyStorage")));
            if (ItemNBTHelper.hasKey(stack, "sideConfig") && GuiScreen.isShiftKeyDown()) {
                int[] s = ItemNBTHelper.getIntArray(stack, "sideConfig");
                String sq = "\u25FC";
                String top = (s[1] == 0
                                ? EnumChatFormatting.BLUE
                                : s[1] == 1 ? EnumChatFormatting.GOLD : EnumChatFormatting.DARK_GRAY)
                        + sq;
                String bot = (s[0] == 0
                                ? EnumChatFormatting.BLUE
                                : s[0] == 1 ? EnumChatFormatting.GOLD : EnumChatFormatting.DARK_GRAY)
                        + sq;
                String front = (s[3] == 0
                                ? EnumChatFormatting.BLUE
                                : s[3] == 1 ? EnumChatFormatting.GOLD : EnumChatFormatting.DARK_GRAY)
                        + sq;
                String back = (s[2] == 0
                                ? EnumChatFormatting.BLUE
                                : s[2] == 1 ? EnumChatFormatting.GOLD : EnumChatFormatting.DARK_GRAY)
                        + sq;
                String left = (s[4] == 0
                                ? EnumChatFormatting.BLUE
                                : s[4] == 1 ? EnumChatFormatting.GOLD : EnumChatFormatting.DARK_GRAY)
                        + sq;
                String right = (s[5] == 0
                                ? EnumChatFormatting.BLUE
                                : s[5] == 1 ? EnumChatFormatting.GOLD : EnumChatFormatting.DARK_GRAY)
                        + sq;
    ...
    public boolean placeBlockAt(
        ...
        if (meta == BlockMetalDevices.META_transformer || meta == BlockMetalDevices.META_transformerHV) {
            ...
            if ((meta != BlockMetalDevices.META_transformer
                    ...
                    && !world.isAirBlock(x, y + 1, z)) return false;
        }
        if (meta == BlockMetalDevices.META_sampleDrill
                && (!world.isAirBlock(x, y + 1, z) || !world.isAirBlock(x, y + 2, z))) return false;
        if (meta == BlockMetalDevices.META_relayHV && world.isAirBlock(x, y + 1, z)) return false;
        if (meta == BlockMetalDevices.META_connectorLV
                || meta == BlockMetalDevices.META_connectorMV
                || meta == BlockMetalDevices.META_connectorHV) {
        ...
        if ((meta == BlockMetalDevices.META_conveyorBelt || meta == BlockMetalDevices.META_conveyorDropper)
                && side != 0
                && side != 1) {
            ...
            if (tileEntity instanceof TileEntityConveyorBelt) {
                ...
                if (con.transportUp
                        && con.facing == ForgeDirection.OPPOSITES[side]
                        && hitY > .75
                        && world.isAirBlock(x, y + 1, z)) y++;
                else if (((con.transportUp && con.facing == side)
                                || (con.transportDown && con.facing == ForgeDirection.OPPOSITES[side]))
                        && hitY <= .125
        ...
        if (tileEntity instanceof TileEntityRelayHV) {
            ...
            if (ItemNBTHelper.hasKey(stack, "energyStorage"))
                ((TileEntityCapacitorLV) tileEntity)
                        .energyStorage.setEnergyStored(ItemNBTHelper.getInt(stack, "energyStorage"));
            ...
            else ((TileEntityCapacitorLV) tileEntity).sideConfig[f] = 1;
        } else if (tileEntity instanceof TileEntityConnectorLV)
            ((TileEntityConnectorLV) tileEntity).facing =
            ...
            if (conveyorFacingPre != -1 || conveyorModePre != -1) {
                ...
                TileEntityConveyorBelt con =
                        (TileEntityConveyorBelt) world.getTileEntity(x + fd.offsetX, y + 1, z + fd.offsetZ);
                if (ForgeDirection.getOrientation(con.facing).equals(fd)) {
                    tile.transportDown = true;
                    f = ForgeDirection.OPPOSITES[f];
                } else tile.transportUp = true;
            } else if (world.getTileEntity(x + fd.offsetX, y - 1, z + fd.offsetZ) instanceof TileEntityConveyorBelt) {
                TileEntityConveyorBelt con =
                        (TileEntityConveyorBelt) world.getTileEntity(x + fd.offsetX, y - 1, z + fd.offsetZ);
                if (ForgeDirection.getOrientation(con.facing).getOpposite().equals(fd)) con.transportDown = true;
            } else if (world.getTileEntity(x - fd.offsetX, y - 1, z - fd.offsetZ) instanceof TileEntityConveyorBelt) {
                TileEntityConveyorBelt con =
                        (TileEntityConveyorBelt) world.getTileEntity(x - fd.offsetX, y - 1, z - fd.offsetZ);
                if (con.facing == f) con.transportUp = true;
            } else if (world.getTileEntity(x - fd.offsetX, y + 1, z - fd.offsetZ) instanceof TileEntityConveyorBelt) {
                TileEntityConveyorBelt con =
                        (TileEntityConveyorBelt) world.getTileEntity(x - fd.offsetX, y + 1, z - fd.offsetZ);
                if (con.facing == f) tile.transportDown = true;
            }
            if (world.getTileEntity(x - fd.offsetX, y, z - fd.offsetZ) instanceof TileEntityConveyorBelt) {
                TileEntityConveyorBelt con =
                        (TileEntityConveyorBelt) world.getTileEntity(x - fd.offsetX, y, z - fd.offsetZ);
                if (con.facing == f && con.transportUp) con.transportUp = false;
            }
            tile.facing = f;
            ...
            for (int i = 1; i <= 2; i++) {
                ...
                if (te instanceof TileEntitySampleDrill) ((TileEntitySampleDrill) te).pos = i;
            }
        }
        return ret;
    }
