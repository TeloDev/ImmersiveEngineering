...
public class TileEntityMetalPress extends TileEntityMultiblockPart implements ISidedInventory, IEnergyReceiver {
    ...
    public void readCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        for (int i = 0; i < processTmp.length; i++)
            if ((process[i] < 0 ^ processTmp[i] < 0) || !descPacket) process[i] = processTmp[i];
        energyStorage.readFromNBT(nbt);
        mold = ItemStack.loadItemStackFromNBT(nbt.getCompoundTag("mold"));
        if (descPacket) active = nbt.getBoolean("active");
        if (nbt.hasKey("stoppedSlot")) stopped = nbt.getInteger("stoppedSlot");
    }
    ...
    public void writeCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        if (this.mold != null) nbt.setTag("mold", this.mold.writeToNBT(new NBTTagCompound()));
        if (descPacket) nbt.setBoolean("active", active);
    ...
    public int getInventoryStackLimit() {
        TileEntityMetalPress master = master();
        if (master != null) return master.getInventoryStackLimit();
        if (stopped >= 0) {
            ...
            if (recipe != null) return recipe.inputSize;
            else {
                stopped = -1;
                stoppedReqSize = -1;
            }
    ...
    public boolean canInsertItem(int slot, ItemStack stack, int side) {
        ...
        if (pos == 3 && side == ForgeDirection.OPPOSITES[facing]) {
            ...
            TileEntityMetalPress master = master();
            if (master.inventory[slot] == null) return stack.stackSize <= master.getInventoryStackLimit();
