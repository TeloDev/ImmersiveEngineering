...
public class TileEntityFluidPump extends TileEntityIEBase implements IFluidHandler, IEnergyReceiver {
    ...
    public FluidStack drain(ForgeDirection from, int maxDrain, boolean doDrain) {
        if (dummy || from == null || from == ForgeDirection.UNKNOWN || sideConfig[from.ordinal()] != 1) return null;
    ...
    public int getEnergyStored(ForgeDirection from) {
        if (dummy) {
            TileEntity te = worldObj.getTileEntity(xCoord, yCoord - 1, zCoord);
            if (te instanceof TileEntityFluidPump) return ((TileEntityFluidPump) te).getEnergyStored(from);
            return 0;
        }
    ...
    public int getMaxEnergyStored(ForgeDirection from) {
        if (dummy) {
            TileEntity te = worldObj.getTileEntity(xCoord, yCoord - 1, zCoord);
            if (te instanceof TileEntityFluidPump) return ((TileEntityFluidPump) te).getMaxEnergyStored(from);
            return 0;
        }
