...
public class TileEntityExcavator extends TileEntityMultiblockPart implements IEnergyReceiver {
    ...
    public void updateEntity() {
        ...
        int[] wheelAxis = {
            xCoord + (facing == 5 ? -4 : facing == 4 ? 4 : 0), yCoord, zCoord + (facing == 3 ? -4 : facing == 2 ? 4 : 0)
        };
        ...
        if (!worldObj.isRemote) {
                ...
                if (((TileEntityBucketWheel) center).facing == ff) {
                    ...
                    if (active != wheel.active)
                        worldObj.addBlockEvent(
                                wheel.xCoord, wheel.yCoord, wheel.zCoord, wheel.getBlockType(), 0, active ? 1 : 0);
                    rot = wheel.rotation;
                    if (rot % 45 > 40) target = Math.round(rot / 360f * 8) % 8;
                }

                // Fix the wheel if necessary
                if (((TileEntityBucketWheel) center).facing != ff
                        || ((TileEntityBucketWheel) center).mirrored != this.mirrored)
                    for (int h = -3; h <= 3; h++)
                        for (int w = -3; w <= 3; w++) {
                            TileEntity te = worldObj.getTileEntity(
                                    wheelAxis[0] + (facing > 3 ? w : 0),
                                    wheelAxis[1] + h,
                                    wheelAxis[2] + (facing < 4 ? w : 0));
            ...
            ExcavatorHandler.MineralMix mineral =
                    ExcavatorHandler.getRandomMineral(worldObj, wheelAxis[0] >> 4, wheelAxis[2] >> 4);
            boolean enabled;
            if (computerControlled) enabled = computerOn;
            else
                enabled = !worldObj.isBlockIndirectlyGettingPowered(
                        xCoord + (facing == 3 ? -1 : facing == 2 ? 1 : 0) * (mirrored ? -1 : 1),
                        yCoord,
                        zCoord + (facing == 4 ? -1 : facing == 5 ? 1 : 0) * (mirrored ? -1 : 1));
                ...
                if (extracted >= consumed) {
                    ...
                    if (target >= 0 && target < 8) {
                        if (wheel.digStacks[(target + 4) % 8] == null) {
                            ...
                            if (blocking != null) {
                                ...
                                worldObj.markBlockForUpdate(wheel.xCoord, wheel.yCoord, wheel.zCoord);
                                    ...
                                    && !worldObj.isAirBlock(
                                            wheel.xCoord + (facing == 5 ? 2 : facing == 4 ? -2 : 0),
                                            wheel.yCoord - 5,
                                            wheel.zCoord + (facing == 3 ? 2 : facing == 2 ? -2 : 0))
                                    && !worldObj.isAirBlock(
                                            wheel.xCoord + (facing == 5 ? -2 : facing == 4 ? 2 : 0),
                                            wheel.yCoord - 5,
                                            wheel.zCoord + (facing == 3 ? -2 : facing == 2 ? 2 : 0))
                                    && !worldObj.isAirBlock(
                                            wheel.xCoord + (facing == 5 ? 1 : facing == 4 ? -1 : 0),
                                            wheel.yCoord - 5,
                                            wheel.zCoord + (facing == 3 ? 1 : facing == 2 ? -1 : 0))
                                    && !worldObj.isAirBlock(
                                            wheel.xCoord + (facing == 5 ? -1 : facing == 4 ? 1 : 0),
                                            wheel.yCoord - 5,
                                            wheel.zCoord + (facing == 3 ? -1 : facing == 2 ? 1 : 0))
                                ...
                                if (ore != null
                                        && configChance > Config.getDouble("excavator_chance")
                                        && failChance > mineral.failChance) {
                        ...
                        if (wheel.digStacks[target] != null) {
                            ...
                            if (b != null && b != Blocks.air) wheel.particleStack = wheel.digStacks[target].copy();
    ...
    public void outputItem(ItemStack stack) {
        ...
        if (stack != null) {
            EntityItem ei = new EntityItem(
                    worldObj, xCoord + .5 + fd.offsetX, yCoord + .5, zCoord + .5 + fd.offsetZ, stack.copy());
    ...
    boolean isInventory(TileEntity tile, int side) {
        if (tile instanceof ISidedInventory && ((ISidedInventory) tile).getAccessibleSlotsFromSide(side).length > 0)
            return true;
        if (tile instanceof IInventory && ((IInventory) tile).getSizeInventory() > 0) return true;
        return false;
    }
    ...
    ItemStack digBlocksInTheWay(TileEntityBucketWheel wheel) {
        ...
        // Backward+Sides
        s = digBlock(
                wheel.xCoord + (facing == 5 ? 1 : facing == 4 ? -1 : 1),
                wheel.yCoord - 4,
                wheel.zCoord + (facing == 3 ? 1 : facing == 2 ? -1 : 1));
        if (s != null) return s;
        s = digBlock(
                wheel.xCoord + (facing == 5 ? 1 : facing == 4 ? -1 : -1),
                wheel.yCoord - 4,
    ...
    ItemStack digBlock(int x, int y, int z) {
        ...
        if (block != null
                ...
                && block.getPlayerRelativeBlockHardness(fakePlayer, worldObj, x, y, z) != 0) {
            if (!block.canHarvestBlock(fakePlayer, meta)) return null;
            ...
            if (block.removedByPlayer(worldObj, fakePlayer, x, y, z, true)) {
                ...
                if (block.canSilkHarvest(worldObj, fakePlayer, x, y, z, meta)) {
                    ...
                    int m = 0;
                    if (bitem != null && bitem.getHasSubtypes()) m = meta;
                    ItemStack itemstack = new ItemStack(bitem, 1, m);
                    if (itemstack != null) items.add(itemstack);

                    ForgeEventFactory.fireBlockHarvesting(
                            items, worldObj, block, x, y, z, meta, 0, 1.0f, true, fakePlayer);
    ...
    public void invalidate() {
        ...
        if (formed && !worldObj.isRemote) {
            ...
            int[] wheelAxis = {
                startX + (facing == 5 ? -4 : facing == 4 ? 4 : 0),
                startY,
                startZ + (facing == 3 ? -4 : facing == 2 ? 4 : 0)
            };
            TileEntity center = worldObj.getTileEntity(wheelAxis[0], wheelAxis[1], wheelAxis[2]);
            if (center instanceof TileEntityBucketWheel) {
        ...
        if (renderAABB == null)
            if (pos == 4)
                renderAABB = AxisAlignedBB.getBoundingBox(
                        xCoord - (facing == 5 ? 5 : facing == 4 ? 0 : 1),
                        yCoord - 1,
                        zCoord - (facing == 3 ? 5 : facing == 2 ? 0 : 1),
                        xCoord + (facing == 4 ? 6 : facing == 5 ? 1 : 2),
                        yCoord + 2,
                        zCoord + (facing == 2 ? 6 : facing == 3 ? 1 : 2));
