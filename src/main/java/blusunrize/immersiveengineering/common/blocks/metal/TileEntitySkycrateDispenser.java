...
// , IBlockOverlayText
{
    ...
    public void updateEntity() {
        if (worldObj.getTotalWorldTime() % 100 == 0) {
            ...
            if (!(worldObj.getTileEntity(cc.posX, cc.posY, cc.posZ) instanceof IImmersiveConnectable)) return;
            ...
            if (outputs != null && outputs.size() > 0) {
                ...
                //						else
                //						{
                //							Vec3 lineVec = Vec3.createVectorHelper(line.end.posX-line.start.posX,
                // line.end.posY-line.start.posY, line.end.posZ-line.start.posZ).normalize();
                //							Vec3 conVec = Vec3.createVectorHelper(c.end.posX-c.start.posX, c.end.posY-c.start.posY,
                // c.end.posZ-c.start.posZ).normalize();
                ...
                if (iicStart != null) vStart = Utils.addVectors(vStart, iicStart.getConnectionOffset(connection));
                if (iicEnd != null) vEnd = Utils.addVectors(vEnd, iicEnd.getConnectionOffset(connection));
                ...
                EntitySkycrate crate = new EntitySkycrate(
                        worldObj, vStart.xCoord, vStart.yCoord, vStart.zCoord, connection, cc0, steps);
                float speed = .2f;
