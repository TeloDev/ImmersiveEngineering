...
public class TileEntityFluidPipe extends TileEntityIEBase implements IFluidHandler, IColouredTile {
    static ConcurrentHashMap<ChunkCoordinates, Set<DirectionalFluidOutput>> indirectConnections =
            new ConcurrentHashMap<ChunkCoordinates, Set<DirectionalFluidOutput>>();
    public static ArrayList<ItemStack> validScaffoldCoverings = new ArrayList<ItemStack>();

    static {
        TileEntityFluidPipe.validScaffoldCoverings.add(
    ...
    public static Set<DirectionalFluidOutput> getConnectedFluidHandlers(ChunkCoordinates node, World world) {
        ...
        Set<DirectionalFluidOutput> fluidHandlers =
                Collections.newSetFromMap(new ConcurrentHashMap<DirectionalFluidOutput, Boolean>());
        openList.add(node);
        while (!openList.isEmpty() && closedList.size() < 1024) {
            ...
            if (world.blockExists(next.posX, next.posY, next.posZ)) {
                ...
                if (!closedList.contains(next)
                        && (te instanceof TileEntityFluidPipe || te instanceof TileEntityFluidPump)) {
                    if (te instanceof TileEntityFluidPipe) closedList.add(next);
                    FluidTankInfo[] tankInfo;
                    for (int i = 0; i < 6; i++) {
                        boolean b = (te instanceof TileEntityFluidPipe)
                                ? (((TileEntityFluidPipe) te).sideConfig[i] == 0)
                                : (((TileEntityFluidPump) te).sideConfig[i] == 1);
                        if (b) {
                            ...
                            if (world.blockExists(
                                ...
                                TileEntity te2 = world.getTileEntity(
                                        next.posX + fd.offsetX, next.posY + fd.offsetY, next.posZ + fd.offsetZ);
                                if (te2 instanceof TileEntityFluidPipe)
                                    openList.add(new ChunkCoordinates(
                                            next.posX + fd.offsetX, next.posY + fd.offsetY, next.posZ + fd.offsetZ));
    ...
    public void invalidate() {
        super.invalidate();
        if (!worldObj.isRemote) indirectConnections.clear();
    }
    ...
    public void readCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        if (nbt.hasKey("colour")) colour = nbt.getInteger("colour");
    }

    @Override
    public void writeCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        nbt.setIntArray("sideConfig", sideConfig);
        if (scaffoldCovering != null) nbt.setTag("scaffold", (scaffoldCovering.writeToNBT(new NBTTagCompound())));
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        if (resource == null
                ...
                || sideConfig[from.ordinal()] != 0
                || worldObj.isRemote) return 0;

        int canAccept = resource.amount;
        if (canAccept <= 0) return 0;

        ArrayList<DirectionalFluidOutput> outputList =
                new ArrayList(getConnectedFluidHandlers(new ChunkCoordinates(xCoord, yCoord, zCoord), worldObj));
        if (outputList.size() < 1) return 0;
        ChunkCoordinates ccFrom =
                new ChunkCoordinates(xCoord + from.offsetX, yCoord + from.offsetY, zCoord + from.offsetZ);
        int sum = 0;
        ...
        for (DirectionalFluidOutput output : outputList) {
            ...
            if (!cc.equals(ccFrom)
                    && worldObj.blockExists(cc.posX, cc.posY, cc.posZ)
                    && output.output.canFill(output.direction.getOpposite(), resource.getFluid())) {
                int limit = (resource.tag != null && resource.tag.hasKey("pressurized"))
                                || canOutputPressurized(output.output, false)
                        ? 1000
                        : 50;
                int tileSpecificAcceptedFluid = Math.min(limit, canAccept);
                int temp = output.output.fill(
                        output.direction.getOpposite(),
                        Utils.copyFluidStackWithAmount(resource, tileSpecificAcceptedFluid, true),
                        false);
                if (temp > 0) {
        ...
        if (sum > 0) {
            ...
            for (DirectionalFluidOutput output : sorting.keySet()) {
                int amount = Math.min(
                        sorting.get(output), canAccept); // fixes issue #1236 on BluSunrize/ImmersiveEngineering
                int r = output.output.fill(
                        output.direction.getOpposite(), Utils.copyFluidStackWithAmount(resource, amount, true), doFill);
                if (r > 50) canOutputPressurized(output.output, true);
                f += r;
                canAccept -= r;
    ...
    boolean canOutputPressurized(IFluidHandler output, boolean consumePower) {
        if (output instanceof TileEntityFluidPump) {
            ...
            if (((TileEntityFluidPump) output).energyStorage.extractEnergy(accelPower, true) >= accelPower) {
                if (consumePower) ((TileEntityFluidPump) output).energyStorage.extractEnergy(accelPower, false);
                return true;
            }
        }
        return false;
    ...
    public byte getConnectionByte() {
        ...
        FluidTankInfo[] tankInfo;
        for (int i = 5; i >= 0; i--) {
            TileEntity con = worldObj.getTileEntity(
                    xCoord + (i == 4 ? -1 : i == 5 ? 1 : 0),
                    yCoord + (i == 0 ? -1 : i == 1 ? 1 : 0),
                    zCoord + (i == 2 ? -1 : i == 3 ? 1 : 0));
            connections <<= 1;
            if (sideConfig[i] == 0 && con instanceof IFluidHandler) {
                tankInfo = ((IFluidHandler) con)
                        .getTankInfo(ForgeDirection.getOrientation(i).getOpposite());
    ...
    public int getConnectionStyle(int connection) {
        ...
        if ((thisConnections & (1 << connection)) == 0) return 0;

        if (thisConnections != 3 && thisConnections != 12 && thisConnections != 48) return 1;
        TileEntity con = worldObj.getTileEntity(
                xCoord + (connection == 4 ? -1 : connection == 5 ? 1 : 0),
                yCoord + (connection == 0 ? -1 : connection == 1 ? 1 : 0),
                zCoord + (connection == 2 ? -1 : connection == 3 ? 1 : 0));
        if (con instanceof TileEntityFluidPipe) {
            byte tileConnections = ((TileEntityFluidPipe) con).getConnectionByte();
            if (thisConnections == tileConnections) return 0;
        }
        return 1;
    }
