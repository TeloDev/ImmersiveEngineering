...
public class ItemBlockClothDevices extends ItemBlockIEBase {
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        ...
        if (ItemNBTHelper.hasKey(stack, "offset")) s += "(+" + ItemNBTHelper.getInt(stack, "offset") + ")";
        return s;
    }
    ...
    public boolean onItemUseFirst(
        ...
        if (!world.isRemote && player.isSneaking()) {
            ...
            offset++;
            if (offset > 5) ItemNBTHelper.remove(stack, "offset");
            else ItemNBTHelper.setInt(stack, "offset", offset);
            return true;
        }
        return false;
    }
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (player.isSneaking()) {
            ...
            if (look.yCoord == max) y += 1;
            else if (-look.yCoord == max) y -= 2;
            else if (look.xCoord == max) x += 1;
            else if (-look.xCoord == max) x -= 1;
            else if (look.zCoord == max) z += 1;
            else if (-look.zCoord == max) z -= 1;
