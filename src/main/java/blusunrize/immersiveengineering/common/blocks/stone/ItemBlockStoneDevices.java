...
public class ItemBlockStoneDevices extends ItemBlockIEBase {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean advInfo) {
        if (stack.getItemDamage() < 4) list.add("This item is deprecated. Hold it in your inventory to update it.");
    }
            ...
            if (meta < 4)
                ((EntityPlayer) ent)
                        .inventory.setInventorySlotContents(
                                slot,
                                new ItemStack(IEContent.blockStoneDecoration, stack.stackSize, stack.getItemDamage()));
        }
    }
