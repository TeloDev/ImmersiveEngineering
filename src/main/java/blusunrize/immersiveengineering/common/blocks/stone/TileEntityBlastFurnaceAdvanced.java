...
public class TileEntityBlastFurnaceAdvanced extends TileEntityBlastFurnace {
    ...
    public float[] getBlockBounds() {
        ...
        if (pos == 7) {
            ...
            float indent = 1;
            if (pos < 9) indent = (pos > 2 && pos < 6) ? .5f : .3125f;
    ...
    public ItemStack getOriginalBlock() {
        if (this.pos == 31) return new ItemStack(Blocks.hopper);
    ...
    protected int getProcessSpeed() {
        ...
        if (heater != null) i += heater.doSpeedup();
    ...
    private TileEntityBlastFurnacePreheater getHeater(boolean first) {
        ...
        if (te instanceof TileEntityBlastFurnacePreheater)
            if (((TileEntityBlastFurnacePreheater) te).facing == phf) return (TileEntityBlastFurnacePreheater) te;
        return null;
    }
