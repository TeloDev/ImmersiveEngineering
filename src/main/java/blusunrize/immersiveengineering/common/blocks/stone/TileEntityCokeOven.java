...
public class TileEntityCokeOven extends TileEntityMultiblockPart implements ISidedInventory, IFluidHandler {
        ...
        if (!worldObj.isRemote && formed && master() == null) {
            ...
            if (process > 0) {
                ...
                if (active) {
                    ...
                    if (recipe != null) {
                        ...
                        if (inventory[1] != null) inventory[1].stackSize += recipe.output.copy().stackSize;
                        else if (inventory[1] == null) inventory[1] = recipe.output.copy();
            ...
            if (tank.getFluidAmount() > 0
                ...
                if (filledContainer != null) {
                    ...
                    else if (inventory[3] == null) inventory[3] = filledContainer.copy();
    ...
    public boolean receiveClientEvent(int id, int arg) {
        if (id == 0) this.formed = arg == 1;
        else if (id == 1) this.active = arg == 1;
        markDirty();
        worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
        return true;
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        ...
        if (slot == 0) return stack != null && CokeOvenRecipe.findRecipe(stack) != null;
        if (slot == 2) return stack != null && FluidContainerRegistry.isEmptyContainer(stack);
        return false;
    }
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        if (!formed) return null;
        TileEntityCokeOven master = master();
        if (master != null) return master.drain(from, resource, doDrain);
        if (resource != null) {
