...
public class TileEntityBlastFurnace extends TileEntityMultiblockPart implements ISidedInventory {
                ...
                if (process <= 0) {
                    if (active) {
                        ...
                        if (recipe != null) {
                            ...
                            if (recipe.slag != null) {
                                if (inventory[3] != null) inventory[3].stackSize += recipe.slag.copy().stackSize;
                                else inventory[3] = recipe.slag.copy();
                            }
                        }
                        processMax = 0;
                        active = false;
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        ...
        if (BlastFurnaceRecipe.isValidBlastFuel(stack)) return slot == 1;
        if (slot == 0) return BlastFurnaceRecipe.findRecipe(stack) != null;
