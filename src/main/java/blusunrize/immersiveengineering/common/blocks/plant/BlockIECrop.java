...
public class BlockIECrop extends BlockBush implements IGrowable {
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        if (metadata >= 4) {
            for (int i = 0; i < 3 + fortune; ++i)
                if (world.rand.nextInt(8) <= metadata) ret.add(new ItemStack(IEContent.itemMaterial, 1, 3));
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        ...
        if (world.getBlockMetadata(x, y, z) < getMaxMeta(0)) world.notifyBlockOfNeighborChange(x, y + 1, z, this);
    }
    ...
    public void updateTick(World world, int x, int y, int z, Random random) {
        ...
        if (light >= 12) {
            ...
            if (random.nextInt((int) (50F / growth) + 1) == 0) {
                ...
                if (meta > 3 && world.isAirBlock(x, y + 1, z)) world.setBlock(x, y + 1, z, this, meta + 1, 0x3);
            }
        }
    }
    ...
    public void func_149853_b(World world, Random rand, int x, int y, int z) {
        ...
        if (meta < getMaxMeta(meta)) {
            ...
            if (newMeta != meta) world.setBlockMetadataWithNotify(x, y, z, newMeta, 0x3);
            meta = newMeta;
        }
        if (meta == 4 && world.isAirBlock(x, y + 1, z)) world.setBlock(x, y + 1, z, this, meta + 1, 0x3);
    }
}
