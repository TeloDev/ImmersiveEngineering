...
public class BlockIESlabs extends BlockIEBase {
    ...
    public boolean isSideSolid(IBlockAccess world, int x, int y, int z, ForgeDirection side) {
        ...
        if (te instanceof TileEntityIESlab) {
            ...
            if (type == 0) return side == DOWN;
            else if (type == 1) return side == UP;
        }
        return true;
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z) {
        ...
        if (te instanceof TileEntityIESlab) {
            int type = ((TileEntityIESlab) te).slabType;
            if (type == 0) this.setBlockBounds(0, 0, 0, 1, .5f, 1);
            else if (type == 1) this.setBlockBounds(0, .5f, 0, 1, 1, 1);
            else this.setBlockBounds(0, 0, 0, 1, 1, 1);
        } else this.setBlockBounds(0, 0, 0, 1, .5f, 1);
    }
