...
public abstract class BlockIEBase extends BlockContainer {
    ...
    public BlockIEBase setMetaLightOpacity(int meta, int opacity) {
        if (meta >= 0 && meta < this.lightOpacities.length) this.lightOpacities[meta] = opacity;
        return this;
    }

    void adjustSound() {
        if (this.blockMaterial == Material.anvil) this.stepSound = Block.soundTypeAnvil;
        ...
        else if (this.blockMaterial == Material.snow) this.stepSound = Block.soundTypeSnow;
        else if (this.blockMaterial == Material.rock) this.stepSound = Block.soundTypeStone;
    ...
    public int getLightOpacity(IBlockAccess world, int x, int y, int z) {
        if (!(world instanceof World)) return getLightOpacity();
        World w = (World) world;
        if (!w.blockExists(x, y, z)) return getLightOpacity();
        int meta = world.getBlockMetadata(x, y, z);
        if (meta >= 0 && meta < this.lightOpacities.length) return this.lightOpacities[meta];
    ...
    public int colorMultiplier(IBlockAccess world, int x, int y, int z) {
        ...
        if (tile instanceof IColouredTile) ((IColouredTile) tile).getColour();
        return 16777215;
    }
    ...
    public IIcon getIcon(int side, int meta) {
        if (meta < icons.length) return icons[meta][getSideForTexture(side)];
        return null;
    }
    ...
    protected int getSideForTexture(int side) {
        if (iconDimensions == 2) return side == 0 || side == 1 ? 0 : 1;
        if (iconDimensions == 4) return side < 2 ? side : side == 2 || side == 3 ? 2 : 3;
    ...
    public boolean isToolEffective(String type, int metadata) {
        if (Lib.TOOL_HAMMER.equals(type) && allowHammerHarvest(metadata)) return true;
    ...
    protected MovingObjectPosition doRayTraceOnBox(
        ...
        if (!this.isVecInsideYZBounds(world, x, y, z, vecMinX, box)) vecMinX = null;
        if (!this.isVecInsideYZBounds(world, x, y, z, vecMaxX, box)) vecMaxX = null;
        if (!this.isVecInsideXZBounds(world, x, y, z, vecMinY, box)) vecMinY = null;
        if (!this.isVecInsideXZBounds(world, x, y, z, vecMaxY, box)) vecMaxY = null;
        if (!this.isVecInsideXYBounds(world, x, y, z, vecMinZ, box)) vecMinZ = null;
        if (!this.isVecInsideXYBounds(world, x, y, z, vecMaxZ, box)) vecMaxZ = null;
        ...
        if (vec38 == null) return null;
        else {
            byte b0 = -1;
            if (vec38 == vecMinX) b0 = 4;
            if (vec38 == vecMaxX) b0 = 5;
            if (vec38 == vecMinY) b0 = 0;
            if (vec38 == vecMaxY) b0 = 1;
            if (vec38 == vecMinZ) b0 = 2;
            if (vec38 == vecMaxZ) b0 = 3;
