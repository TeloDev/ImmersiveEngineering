...
public class ItemNBTHelper {
    ...
    public static void remove(ItemStack stack, String key) {
        if (hasKey(stack, key)) {
            ...
            if (getTag(stack).hasNoTags()) stack.setTagCompound(null);
        }
    }
    ...
    public static int handleDelayedSoundsForStack(ItemStack stack, String nbtKey, Entity ent) {
        if (!hasKey(stack, "delayedSound_" + nbtKey + "_delay")) return -1;
    ...
    public static FluidStack getFluidStack(ItemStack stack, String key) {
        if (hasTag(stack)) {
            ...
            if (FluidRegistry.getFluid(name) != null) return new FluidStack(FluidRegistry.getFluid(name), amount);
        }
        return null;
    }
