...
public class SqueezerDriver extends DriverTileEntity {
    ...
    public ManagedEnvironment createEnvironment(World w, int x, int y, int z) {
        ...
        if (te instanceof TileEntitySqueezer) {
            ...
            return new FermenterEnvironment(
                    w,
                    squeezer.xCoord - squeezer.offset[0],
                    squeezer.yCoord - squeezer.offset[1],
                    squeezer.zCoord - squeezer.offset[2]);
        }
        return null;
    }
    ...
    public class FermenterEnvironment extends ManagedEnvironmentIE<TileEntitySqueezer> {
        ...
        @Callback(
                doc = "function(slot:int):table, table, table, int -- returns the recipe for the specified input slot")
        public Object[] getRecipe(Context context, Arguments args) {
            int slot = args.checkInteger(0);
            if (slot < 1 || slot > 9) throw new IllegalArgumentException("Input slots are 1-9");
