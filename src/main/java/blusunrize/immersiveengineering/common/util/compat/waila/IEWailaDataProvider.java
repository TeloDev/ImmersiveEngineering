    ...
    public List<String> getWailaBody(
        ...
        if (b instanceof BlockIECrop) {
            ...
            if (min == max)
                currenttip.add(String.format(
                        "%s : %s",
                        StatCollector.translateToLocal("hud.msg.growth"),
                        StatCollector.translateToLocal("hud.msg.mature")));
            else {
                float growth = ((meta - min) / (float) (max - min)) * 100f;
                if (growth < 100.0)
                    currenttip.add(
                            String.format("%s : %.0f %%", StatCollector.translateToLocal("hud.msg.growth"), growth));
                else
                    currenttip.add(String.format(
                            "%s : %s",
                            StatCollector.translateToLocal("hud.msg.growth"),
        ...
        if (tile instanceof TileEntityWoodenBarrel) {
            ...
            if (!tank.hasKey("Empty")) {
                ...
                currenttip.add(String.format(
                        "%s: %d / %d mB",
                        new Object[] {fluid.getLocalizedName(), Integer.valueOf(fluid.amount), 12000}));
            } else currenttip.add(StatCollector.translateToLocal("hud.msg.empty"));
        }
        return currenttip;
    }
