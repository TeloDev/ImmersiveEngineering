...
public class AssemblerDriver extends DriverTileEntity {
    ...
    public ManagedEnvironment createEnvironment(World w, int x, int y, int z) {
        ...
        if (te instanceof TileEntityAssembler) {
            ...
            if (offsetY == -1) {
                ...
                return new AssemblerEnvironment(
                        w,
                        assembler.xCoord - assembler.offset[0],
                        assembler.yCoord - assembler.offset[1],
                        assembler.zCoord - assembler.offset[2],
    ...
    public class AssemblerEnvironment extends ManagedEnvironmentIE<TileEntityAssembler> {
        ...
        @Callback(
                doc =
                        "function(recipe:int):boolean -- get whether the ingredients for the specified recipe are available")
        public Object[] hasIngredients(Context context, Arguments args) {
            int recipe = args.checkInteger(0);
            if (recipe > 3 || recipe < 1) throw new IllegalArgumentException("Only recipes 1-3 are available");
            TileEntityAssembler master = getTileEntity();
            ...
            for (ItemStack stack : master.inventory) if (stack != null) queryList.add(stack.copy());
        ...
        public Object[] getRecipe(Context context, Arguments args) {
            ...
            for (int i = 0; i < 9; i++) ret.put("in" + (i + 1), te.patterns[recipe - 1].inv[i]);
        ...
        @Callback(
                doc =
                        "function(recipe:int):boolean -- check whether the recipe in the specified position has an output")
        ...
        public Object[] getTank(Context context, Arguments args) {
            ...
            if (tank > 3 || tank < 1) throw new IllegalArgumentException("Only tanks 1-3 are available");
        ...
        public Object[] getStackInSlot(Context context, Arguments args) {
            ...
            if (slot < 1 || slot > 18) throw new IllegalArgumentException("Only slots 1-18 are available");
        ...
        public Object[] getBufferStack(Context context, Arguments args) {
            int slot = args.checkInteger(0);
            if (slot < 1 || slot > 3) throw new IllegalArgumentException("Only recipes 1-3 are available");
