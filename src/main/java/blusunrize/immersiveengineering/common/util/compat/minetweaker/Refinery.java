...
public class Refinery {
    ...
    public static void removeRecipe(ILiquidStack output) {
        if (MTHelper.toFluidStack(output) != null) MineTweakerAPI.apply(new Remove(MTHelper.toFluidStack(output)));
    }
    ...
    private static class Remove implements IUndoableAction {
        ...
        public void undo() {
            if (removedRecipes != null)
                for (RefineryRecipe recipe : removedRecipes) if (recipe != null) DieselHandler.refineryList.add(recipe);
        }
