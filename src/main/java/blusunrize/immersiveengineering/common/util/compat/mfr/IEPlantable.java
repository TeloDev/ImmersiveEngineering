...
public class IEPlantable implements IFactoryPlantable {
    ...
    public ReplacementBlock getPlantedBlock(World world, int x, int y, int z, ItemStack stack) {
        if (stack.getItem() != seeds) return new ReplacementBlock(Blocks.air);
    ...
    public void prePlant(World world, int x, int y, int z, ItemStack stack) {
        Block ground = world.getBlock(x, y - 1, z);
        if (ground.equals(Blocks.grass) || ground.equals(Blocks.dirt)) world.setBlock(x, y - 1, z, Blocks.farmland);
    }
