...
public class BotaniaHelper extends IECompatModule {
    ...
    public void init() {
        if (Config.getBoolean("hardmodeBulletRecipes"))
            BlueprintCraftingRecipe.addRecipe(
                    "specialBullet",
                    new ItemStack(IEContent.itemBullet, 1, 7),
        ...
        BlueprintCraftingRecipe.addRecipe(
                ...
                Items.gunpowder,
                new ItemStack(IEContent.itemBullet, 4, 7));
        ...
        ShaderRegistry.registerShader(
                ...
                "5",
                EnumRarity.epic,
                new int[] {26, 26, 40, 220},
                new int[] {0, 70, 49, 220},
                new int[] {40, 40, 50, 220},
                new int[] {5, 10, 8, 180},
                null,
                false,
                true);
    ...
    public static class ThreadContributorToNameFormatter extends Thread {
        ...
        public void run() {
            ...
            for (String uuid : ItemRevolver.specialRevolvers.keySet())
                nameToSpecial.putAll(
                        ImmersiveEngineering.proxy.getNameFromUUID(uuid).toLowerCase(),
                        ItemRevolver.specialRevolvers.get(uuid));
        }
    }
        ...
        if (!event.isCanceled() && event.entityLiving.getClass().getName().endsWith("EntityDoppleganger")) {
            ...
            if (tag.getBoolean("hardMode"))
                for (EntityItem item : event.drops)
                    if (item != null
                            && item.getEntityItem() != null
                            && IEContent.itemShaderBag.equals(
    ...
    public void onPotatoRender(TinyPotatoRenderEvent event) {
        if (event.tile.getWorldObj() == null) return;
        if (revolverEntity == null) {
            revolverEntity =
                    new EntityItem(event.tile.getWorldObj(), 0.0D, 0.0D, 0.0D, new ItemStack(IEContent.itemRevolver));
            ...
            if (formattedName.equalsIgnoreCase("Mr Damien Hazard") || event.name.equalsIgnoreCase("Mr Hazard"))
                ...
                if (nameToSpecial.containsKey(event.name.toLowerCase())) {
                    ...
                    if (list != null && list.size() > 0) {
                        long ticks = event.tile.getWorldObj() != null
                                ? event.tile.getWorldObj().getTotalWorldTime() / 100
                                : 0;
