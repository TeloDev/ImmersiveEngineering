...
public class PeripheralFermenter extends IEPeripheral {
    ...
    public Object[] callMethod(IComputerAccess computer, ILuaContext context, int method, Object[] arguments)
        ...
        if (te == null) throw new LuaException("The fermenter was removed");
        switch (method) {
            case 0: // recipe
                if (arguments.length != 1 || !(arguments[0] instanceof Double))
                    throw new LuaException("Wrong amount of arguments, needs one integer");
                int slot = (int) (double) arguments[0];
                if (slot < 1 || slot > 9) throw new LuaException("Input slots are numbers 1-9");
                FermenterRecipe recipe = DieselHandler.findFermenterRecipe(te.getStackInSlot(slot - 1));
                if (recipe != null)
                    return new Object[] {
                        saveStack(te.getStackInSlot(slot - 1)),
                        saveStack(recipe.output),
                        saveFluidStack(recipe.fluid),
                        recipe.time
                    };
                else return new Object[] {"No recipe found"};
            case 1: // Input stack
                if (arguments.length != 1 || !(arguments[0] instanceof Double))
                    throw new LuaException("Wrong amount of arguments, needs one integer");
