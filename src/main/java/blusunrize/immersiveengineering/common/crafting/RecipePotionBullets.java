...
public class RecipePotionBullets implements IRecipe {
    ...
    public boolean matches(InventoryCrafting inv, World world) {
        ...
        for (int i = 0; i < inv.getSizeInventory(); i++) {
            ...
            if (stackInSlot != null)
                if (bullet == null
                        && IEContent.itemBullet.equals(stackInSlot.getItem())
                        && stackInSlot.getItemDamage() == 10) bullet = stackInSlot;
                else if (potion == null && stackInSlot.getItem() instanceof ItemPotion) potion = stackInSlot;
                else return false;
        }
