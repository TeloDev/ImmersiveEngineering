...
public class ArcRecyclingRecipe extends ArcFurnaceRecipe {
    ...
    public ItemStack[] getOutputs(ItemStack input, ItemStack[] additives) {
        if (outputs == null) return new ItemStack[0];
        float mod = !input.getItem().isDamageable()
                ? 1
                : (input.getMaxDamage() - input.getItemDamage()) / (float) input.getMaxDamage();
        ...
        for (Entry<ItemStack, Double> e : outputs.entrySet()) {
            ...
            if (scaledOut >= 1) outs.add(Utils.copyStackWithAmount(e.getKey(), (int) scaledOut));
    ...
    public boolean matches(ItemStack input, ItemStack[] additives) {
        if (input != null && this.input instanceof ItemStack) {
            ...
            return input.getItem().equals(inStack.getItem())
                    && (ignoreMeta || inStack.getItemDamage() == input.getItemDamage());
        }
        return false;
    }
}
