...
public class IEVillagerTradeHandler implements IVillageTradeHandler {
    ...
    void addDeal(float chance, Object... objects) {
        ...
        if (items[0] != null && items[1] != null)
            if (items[2] != null) dealList.add(new MerchantDeal(chance, items[0], items[1], items[2]));
            else dealList.add(new MerchantDeal(chance, items[0], null, items[1]));
    }
    ...
    public static class MerchantItem {
        ...
        public MerchantItem(Object item, int minAmount, int maxAmount) {
            if (item instanceof Item) this.item = new ItemStack((Item) item);
            else if (item instanceof Block) this.item = new ItemStack((Block) item);
            else this.item = (ItemStack) item;
        ...
        public ItemStack getItem(Random rand) {
            if (minAmount == maxAmount) return item;
